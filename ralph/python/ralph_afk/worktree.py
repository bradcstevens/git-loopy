"""``ralph_afk.worktree`` — the per-Lane worktree setup seam (#65, ADR-0008).
...
The knob is ``COPILOOP_WORKTREE_SETUP``: when set, its command is run in each
...
Like :mod:`ralph_afk.gate`, the setup is a **real, injectable seam**: callers hold
...
the ``ralph_afk.loop._make_worktree_setup`` factory, exactly like the git / gh /
gate factories.
...
* **Configured wins over detected.** An explicit ``COPILOOP_WORKTREE_SETUP`` is a
...
* **Best-effort, not exhaustive.** The detector recognises the common lockfiles /
  ...
  the operator sets ``COPILOOP_WORKTREE_SETUP`` explicitly.
* **Shell execution.** Commands run through the shell (``shell=True``) exactly as
  an operator would type them — the same trust model as :mod:`ralph_afk.gate`
...
class SetupResult:
    ...
    Attributes:
        command: The command that ran, or ``None`` when neither a configured
            ``COPILOOP_WORKTREE_SETUP`` nor the auto-detect produced one (a
...
class CommandWorktreeSetup:
    ...
    Runs the configured ``COPILOOP_WORKTREE_SETUP`` command when one is given,
