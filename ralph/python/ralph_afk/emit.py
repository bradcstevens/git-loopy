"""``ralph_afk.emit`` — the ``EventEmitter`` scrub-and-fan-out seam (issue #43).
...
per-**Iteration** :class:`~ralph_afk.session.IterationSession` each ran their own
copy of the pipeline — *compose -> write the replay JSONL -> fan out to the sink
list* — and the two copies disagreed:

* :meth:`IterationSession._record <ralph_afk.session.IterationSession._record>`
  scrubs the envelope **once**, then hands the *scrubbed* dict to both the
  :class:`~ralph_afk.persist.EventLogWriter` and the
  :class:`~ralph_afk.sinks.SinkFanout`. Correct.
* ``_Loop._emit`` (``ralph_afk/loop.py``) writes via the ``EventLogWriter`` —
  ...
  (``ralph_afk/sinks.py``). The replay log is clean; the live Renderer /
...
* **Deep and pure.** Imports stay ``__future__`` + :mod:`typing` +
  :mod:`ralph_afk.events` only — no ``rich`` / ``textual`` / SDK / ``persist`` /
...
from ralph_afk.events import make_event, scrub
...
class _EventLog(Protocol):
    ...
    :class:`~ralph_afk.persist.EventLogWriter` satisfies this structurally.
    """
...
class _Sinks(Protocol):
    ...
    :class:`~ralph_afk.sinks.SinkFanout` satisfies this structurally.
    """
...
class EventEmitter:
    ...
    exposing ``render(dict)`` (the :class:`~ralph_afk.sinks.SinkFanout`), and
    ...
    def emit(
        ...
        **payload: Any,
        ...
        Composes via :func:`ralph_afk.events.make_event` (``iter=iter_num``) and
    ...
    def dispatch(self, envelope: dict[str, Any]) -> None:
        ...
        The single :func:`ralph_afk.events.scrub` call runs *before* the guards,
