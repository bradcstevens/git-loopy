"""``ralph_afk.sinks`` — event sink protocol + swappable fan-out (issue #22).

The AFK loop and its per-iteration
:class:`~ralph_afk.session.IterationSession` no longer hand events to the
line-printer :class:`~ralph_afk.ui.renderer.Renderer` directly. They dispatch
...
class EventSink(Protocol):
    ...
    The line-printer :class:`~ralph_afk.ui.renderer.Renderer` satisfies this
