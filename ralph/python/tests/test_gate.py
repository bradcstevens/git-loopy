"""Tests for ``ralph_afk.gate`` — the runner-side Integration gate seam (#60).
...
load-bearing quality gate; :mod:`ralph_afk.gate` is what runs them *from the runner
side* and reports green/red.

Two layers are exercised here:

* :func:`ralph_afk.gate.parse_feedback_loops` — a pure parser of the ``AGENTS.md``
  ``## Feedback loops`` table (platform-independent).
* :class:`ralph_afk.gate.AgentsMdGateRunner` — the production adapter, exercised
...
def test_make_gate_runner_is_monkeypatchable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("ralph_afk.loop._make_gate_runner", lambda: sentinel)
