"""Tests for ``ralph_afk.emit`` (issue #43 — the ``EventEmitter`` seam).

:class:`~ralph_afk.emit.EventEmitter` is the single seam that composes, scrubs,
...
``render`` receives an already-scrubbed envelope (``ralph_afk/sinks.py``).
...
* The module imports only ``{__future__, typing, ralph_afk.events}`` (AST guard,
...
from ralph_afk import emit as emit_module
from ralph_afk.emit import EventEmitter
from ralph_afk.events import REDACTED_SECRET, make_event, scrub
...
def test_emit_module_imports_are_constrained() -> None:
    ...
    allow = {"__future__", "typing", "ralph_afk.events"}
    seen: set[str] = set()
    ...
    assert "ralph_afk.sinks" not in seen
    assert "ralph_afk.persist" not in seen
