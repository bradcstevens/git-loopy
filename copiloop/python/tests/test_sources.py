"""Tests for :mod:`copiloop.sources` — IssueSource Protocol + impls."""
...
from copiloop import sources as sources_module
...
def _silent_logger() -> logging.Logger:
    ...
    logger = logging.getLogger(f"copiloop.tests.{id(object())}")
...
class _FakeCommit:
    """Minimal stand-in for :class:`copiloop.git.Commit`."""
...
class TestModuleStructure:
    ...
    def test_imports_are_constrained(self) -> None:
        """sources.py may only import stdlib + copiloop.{gh,git,wrapper}.

        Forbidden: copilot SDK, rich, copiloop.{loop,cli,config,session,
        ...
        allowed_copiloop_submodules = {"gh", "git", "wrapper"}
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if top == "copiloop":
                        ...
                        if sub and sub not in allowed_copiloop_submodules:
                            offenders.append(
                ...
                if top == "copiloop":
                    ...
                    if len(parts) >= 2 and parts[1] not in allowed_copiloop_submodules:
                        offenders.append(
        ...
        assert not offenders, (
            "copiloop/sources.py has forbidden imports:\n  "
            + "\n  ".join(offenders)
        )
