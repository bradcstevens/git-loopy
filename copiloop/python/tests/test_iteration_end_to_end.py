"""End-to-end integration test for :mod:`copiloop.loop`.

Drives one full ``copiloop`` iteration with the SDK and git/gh seams
...
* ``copiloop.loop._make_client`` → a :class:`FakeCopilotClient`
  (reused from :mod:`tests.test_session`) that scripts a single
  iteration's SDK event flow.
* ``copiloop.loop._make_github_client`` → a single
  :class:`~tests.fakes.FakeGitHubClient` (issue #47) injected through the
  loop's ``gh`` seam, replacing the old per-function ``copiloop.gh.*``
...
* ``copiloop.loop._make_git_client`` → a single
  :class:`~tests.fakes.FakeGitClient` (issue #46) injected through the loop's
  git seam, replacing the old per-function ``copiloop.git.*`` monkeypatches.
...
* Return code 0.
* ``.copiloop/logs/<stem>.jsonl`` exists and every line is envelope-
  conformant JSON.
* ``.copiloop/runs/<stem>.json`` exists and matches the persist schema
  (one iteration row, expected counts).
* ``.gitignore`` contains ``.copiloop/`` (the persist factory touches it).
...
from copiloop.events import REDACTED_SECRET
from copiloop.persist import WritersBundle, create_writers
from copiloop.pricing import Pricing
from copiloop.sinks import SinkFanout
from copiloop.ui import RunSummary
from copiloop.wrapper import is_checkpoint_message
...
def _logged_types(tmp_path: Path) -> list[str]:
    ...
    logs_dir = tmp_path / ".copiloop" / "logs"
...
def test_loop_runs_one_iteration_end_to_end(tmp_path, monkeypatch) -> None:
    ...
    * tmp_path as the repo root with a ``copiloop/prompt.md`` and a
    ...
    * Mocked git: one new commit between pre and post HEAD; commit
    ...
    (tmp_path / "copiloop" / "prompt.md").write_text(
    ...
    assert ".copiloop/" in gitignore.splitlines()
    ...
    runs_dir = tmp_path / ".copiloop" / "runs"
...
def test_loop_prds_end_to_end_one_iteration(tmp_path, monkeypatch) -> None:
    ...
    * The worktree (``prds/`` tree) is unchanged after the run —
    ...
    # Run-summary JSON.
    json_files = list((tmp_path / ".copiloop" / "runs").glob("*.json"))
    assert len(json_files) == 1
    payload = json.loads(json_files[0].read_text(encoding="utf-8"))
    assert len(payload["iterations"]) == 1
    iter_row = payload["iterations"][0]
...
def test_loop_send_and_wait_exception_is_no_progress(tmp_path, monkeypatch) -> None:
    ...
    log_files = list((tmp_path / ".copiloop" / "logs").glob("*.jsonl"))
    assert len(log_files) == 1
    types_seen = {
        json.loads(line)["type"]
        for line in log_files[0].read_text(encoding="utf-8").splitlines()
...
def test_loop_bad_pricing_file_returns_exit_one(tmp_path, monkeypatch) -> None:
    """A malformed ``COPILOOP_PRICING_FILE`` override surfaces as exit 1.
...
def test_loop_emits_otel_span_tree_when_enabled(tmp_path, monkeypatch) -> None:
    ...
    # Wipe sticky-enable cache + flip COPILOOP_OTEL_ENABLED so the seam
    # picks up the externally-installed provider on its first init().
    telemetry.reset_for_tests()
    monkeypatch.setenv("COPILOOP_OTEL_ENABLED", "1")
    ...
    # (and its copiloop.enforce_closures span) fires as before.
    fake_client.on_send = lambda: fake_git.simulate_agent_commit(
        sha="abcdef1234567890abcdef1234567890abcdef12",
        subject="feat: stuff",
        body="Closes #42",
    ...
    run_span = by_name["copiloop.run"][0]
    iter_span = by_name["copiloop.iteration"][0]
    collect_span = by_name["copiloop.collect_issues"][0]
    session_span = by_name["copiloop.session"][0]
    closures_span = by_name["copiloop.enforce_closures"][0]

    # Parent relationships: run → iteration → {collect, session, closures}.
    assert run_span.parent is None, "copiloop.run is the root span"
    assert iter_span.parent is not None
    assert iter_span.parent.span_id == run_span.context.span_id, (
        "copiloop.iteration must nest under copiloop.run"
    )
    for child in (collect_span, session_span, closures_span):
        assert child.parent is not None
        assert child.parent.span_id == iter_span.context.span_id, (
            f"{child.name!r} must nest under copiloop.iteration; "
...
def test_loop_pr_advance_emits_pr_advanced_event(tmp_path, monkeypatch) -> None:
    ...
    * the base branch is never switched (HEAD already on base).
    ...
    jsonl_files = list((tmp_path / ".copiloop" / "logs").glob("*.jsonl"))
    assert len(jsonl_files) == 1
    types_seen: list[str] = []
...
class _NoopSource:
    """Inert :class:`~copiloop.sources.IssueSource` stand-in.
