"""Tests for ``copiloop.interactive.state`` (issue #23 — live run model).

:class:`~copiloop.interactive.state.LiveRunState` is the Textual-agnostic
...
* :func:`~copiloop.interactive.state.format_header` content;
* structural conformance to the :class:`copiloop.sinks.EventSink` protocol;
...
from copiloop import sinks as sinks_module
from copiloop.interactive import state as state_module
from copiloop.interactive.state import LiveRunState, format_header
...
def test_state_module_imports_are_constrained() -> None:
    ...
    import-guard convention, mirroring ``copiloop.sinks``). The **only**
    first-party import allowed is :mod:`copiloop.usage` (issue #41) — the shared
    ``UsageTally`` **Consumption** value object the per-Active-issue accrual folds
    onto. It is itself deep and pure (stdlib + :mod:`copiloop.pricing`), so
    ...
    allow = {
        ...
        # Consumption value object. Deep and pure (stdlib + copiloop.pricing);
    ...
    assert "copiloop.usage" in seen, "state.py folds Consumption through UsageTally"
    assert "copiloop.pricing" not in seen, "state.py imports usage, not pricing"
