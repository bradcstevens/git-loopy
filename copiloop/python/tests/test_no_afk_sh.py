...
ADR-0007 makes the ``copiloop`` Python CLI the single, canonical entrypoint and
**deletes** the ``afk.sh`` convenience launcher that used to hard-code
``COPILOOP_MODEL=claude-opus-4.8 COPILOOP_REASONING_EFFORT=max``. Model and
...
* the ``copiloop/afk.sh`` file must not come back, and
...
def test_afk_sh_launcher_is_deleted() -> None:
    """The ``copiloop/afk.sh`` launcher file must not exist (ADR-0007 hard cut)."""
    repo_root = _find_repo_root()
    if repo_root is None:
        pytest.skip("repo root not found (installed-wheel run) -- nothing to check")
    assert not (repo_root / "copiloop" / "afk.sh").exists(), (
        "copiloop/afk.sh was deleted by ADR-0007 (single Python entrypoint) -- "
...
def test_no_afk_sh_references_in_tracked_files() -> None:
    ...
    assert not failures, (
        ...
        "`copiloop` the single entrypoint and deleted afk.sh. Launch with "
        "`copiloop` (or `uvx copiloop`) and set model/effort via --model / "
