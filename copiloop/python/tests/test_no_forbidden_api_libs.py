"""Static guard: no Python-native API libraries in ``copiloop/``.
...
Our own ``copiloop.git`` module is **not** affected by this guard because
its top-level package name is ``copiloop``, not ``git`` — qualified
imports (``from copiloop import git``, ``from copiloop.git import head_sha``,
``import copiloop.git``) and relative imports (``from . import git``) all
target ``copiloop``.

The check is performed via AST inspection across every ``*.py`` under
``copiloop/`` so any future module addition is automatically covered.
"""
...
def _iter_package_python_files() -> list[Path]:
    """Return every ``.py`` file under the installed ``copiloop`` package."""
    pkg_dir = Path(copiloop.__file__).parent
...
def test_at_least_one_module_is_inspected() -> None:
    ...
    assert files, "expected to find Python modules under copiloop/ to inspect"


def test_no_forbidden_api_libraries_in_any_copiloop_module() -> None:
    ...
    * ``from copiloop import git`` — qualified, top-level package is ``copiloop``.
    * ``import copiloop.git`` — same.
    * ``from . import git`` / ``from .git import head_sha`` — relative imports
    ...
    assert not failures, (
        "copiloop/ must funnel every external API call through subprocess "
        "wrappers around `gh` and `git` — Python-native API libraries are "
        "forbidden:\n  " + "\n  ".join(failures)
    )


def test_our_own_copiloop_git_module_is_not_misidentified() -> None:
    """Regression guard for the allowlist logic.

    Imports of our own ``copiloop.git`` module — qualified or relative —
    ...
    """
    samples = [
        "from copiloop import git",
        "from copiloop.git import head_sha",
        "import copiloop.git",
        "from . import git",
    ...
    for src in samples:
        ...
        if isinstance(node, ast.Import):
            ...
            assert top == "copiloop", f"{src!r} classified wrongly: top={top!r}"
        elif isinstance(node, ast.ImportFrom):
            if node.level > 0:
