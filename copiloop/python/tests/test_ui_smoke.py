"""Tests for ``copiloop.ui`` (issue #8 — Rich UI: console + renderer + summary).
...
from copiloop.pricing import ModelPricing, Pricing
from copiloop.ui import IterationSnapshot, Renderer, RunSummary, get_console
from copiloop.ui.console import STYLES
...
def test_iteration_snapshot_to_counters_kwargs_conversion() -> None:
    ...
    itself) keeps the UI module's import graph free of ``copiloop.persist``
...
_ALLOWED_UI_IMPORTS: frozenset[str] = frozenset(
    {
        ...
        "copiloop.events",
        "copiloop.pricing",
        # copiloop.usage — the shared Consumption value object (issue #40).
        # Deep and pure (stdlib + copiloop.pricing); summary.py folds its
...
def _classify_import(node: ast.AST, current_module: str | None) -> list[str]:
    ...
    returns ``["a.b", "c"]``. Relative imports inside ``copiloop.ui`` are
    exempt (returns ``[]``).
    """
...
def test_ui_module_imports_are_constrained(path: Path) -> None:
    """``copiloop.ui.*`` modules import only Rich, stdlib, and first-party deep modules.

    Catches stray third-party imports (httpx, requests, github, gitpython)
    and accidental coupling to shell-side modules (``copiloop.gh``,
    ``copiloop.git``, ``copiloop.persist``) which the UI must not import
