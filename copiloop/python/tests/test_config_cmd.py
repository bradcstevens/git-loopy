"""Tests for :mod:`copiloop.configcmd` — the ``copiloop config`` subcommand group.
...
default) via :func:`copiloop.cli.resolve_config`.
...
from copiloop import configcmd, settings
...
def test_get_env_overrides_project(tmp_path: Path) -> None:
    ...
    configcmd.run_get(
        "model",
        repo_root=tmp_path,
        env=_env(tmp_path, COPILOOP_MODEL="claude-opus-4.7"),
        out=out,
        err=_Sink(),
    )
...
def test_get_denylist_is_union_across_sources(tmp_path: Path) -> None:
    ...
    configcmd.run_get(
        "deny_tools",
        repo_root=tmp_path,
        env=_env(tmp_path, COPILOOP_DENY_TOOLS="bash"),
        out=out,
        err=_Sink(),
    )
