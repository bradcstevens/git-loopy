"""Tests for ``copiloop.emit`` (issue #43 — the ``EventEmitter`` seam).

:class:`~copiloop.emit.EventEmitter` is the single seam that composes, scrubs,
...
``render`` receives an already-scrubbed envelope (``copiloop/sinks.py``).
...
* :meth:`EventEmitter.emit` composes via :func:`copiloop.events.make_event`
...
* The module imports only ``{__future__, typing, copiloop.events}`` (AST guard,
...
from copiloop import emit as emit_module
from copiloop.emit import EventEmitter
from copiloop.events import REDACTED_SECRET, make_event, scrub
...
def test_emit_module_imports_are_constrained() -> None:
    ...
    allow = {"__future__", "typing", "copiloop.events"}
    seen: set[str] = set()
    ...
    assert "copiloop.sinks" not in seen
    assert "copiloop.persist" not in seen
