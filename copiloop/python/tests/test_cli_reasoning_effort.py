...
from copiloop.cli import _derive_reasoning_effort_from_model
...
def _clear_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for name in (
        ...
        "COPILOOP_ISSUE_SOURCE",
        "COPILOOP_MAX_NMT_STRIKES",
        "COPILOOP_DENY_TOOLS",
        "COPILOOP_DENY_SKILLS",
...
def test_main_strips_effort_suffix_to_base_model_id(
    ...
    """
    monkeypatch.setenv("COPILOOP_MODEL", "claude-opus-4.7-xhigh")
    captured: list[RunConfig] = []
    _install_fake_runner(monkeypatch, captured, tmp_path)
...
def test_main_forces_none_effort_for_reasoning_incapable_model(
    ...
    """
    monkeypatch.setenv("COPILOOP_MODEL", "claude-haiku-4.5")
...
def test_main_accepts_max_effort(
    ...
    monkeypatch.setenv("COPILOOP_REASONING_EFFORT", "max")
    captured: list[RunConfig] = []
    _install_fake_runner(monkeypatch, captured, tmp_path)
...
def test_main_unknown_model_passes_through_with_warning(
    ...
    """
    monkeypatch.setenv("COPILOOP_MODEL", "some-future-model-9")
    captured: list[RunConfig] = []
    _install_fake_runner(monkeypatch, captured, tmp_path)
...
def test_main_leaves_reasoning_effort_unset_for_non_pinned_model(
    ...
    """
    monkeypatch.setenv("COPILOOP_MODEL", "claude-sonnet-4.6")
    captured: list[RunConfig] = []
    _install_fake_runner(monkeypatch, captured, tmp_path)
...
def test_main_rejects_invalid_reasoning_effort_env(
    ...
    """
    monkeypatch.setenv("COPILOOP_REASONING_EFFORT", "ultra")
    captured: list[RunConfig] = []
    _install_fake_runner(monkeypatch, captured, tmp_path)
