...
* :func:`copiloop.cli._resolve_include_prs` reads the ``INCLUDE_PRS`` env
  override and returns ``True`` / ``False`` / ``None`` (no override).
* :func:`copiloop.loop._resolve_include_prs` applies precedence: an explicit
