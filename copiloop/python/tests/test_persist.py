"""Tests for ``copiloop.persist`` (issue #7).
...
* :func:`make_run_id` — 26-char Crockford-base32 ULID, deterministic
  ...
  prefix sortably.
* :func:`ensure_gitignore_entry` — appends ``.copiloop/`` when missing,
  no-ops when ``.gitignore`` absent, idempotent when the entry already
  exists in either ``.copiloop/`` or ``.copiloop`` form.
...
* Module imports only stdlib + ``copiloop.events`` (AST guard).
"""
...
from copiloop import persist as persist_module
from copiloop.events import REDACTED_SECRET, make_event
...
def test_ensure_gitignore_entry_appends_when_missing(tmp_path: Path) -> None:
    ...
    assert ".copiloop/" in content
    # Should be appended after the existing content.
    assert content.rstrip().splitlines()[-1] == ".copiloop/"
...
def test_ensure_gitignore_entry_adds_leading_newline_when_no_trailing_newline(
    ...
    assert lines == ["node_modules", ".copiloop/"]
...
def test_ensure_gitignore_entry_idempotent_when_copiloop_with_slash_present(
    tmp_path: Path,
) -> None:
    gi = tmp_path / ".gitignore"
    gi.write_text("node_modules/\n.copiloop/\n", encoding="utf-8")
    ...
    content = gi.read_text(encoding="utf-8")
    assert content.count(".copiloop/") == 1


def test_ensure_gitignore_entry_idempotent_when_copiloop_without_slash_present(
    tmp_path: Path,
) -> None:
    """Both ``.copiloop/`` and ``.copiloop`` are recognised — gitignore treats
    directory-only and "either type" forms as equivalent for our purposes."""
    gi = tmp_path / ".gitignore"
    gi.write_text(".copiloop\n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    content = gi.read_text(encoding="utf-8")
    # Should NOT append a duplicate '.copiloop/' line.
    assert content.count(".copiloop") == 1
    assert ".copiloop/" not in content
...
def test_ensure_gitignore_entry_tolerates_whitespace_around_existing_line(
    ...
    gi.write_text("  .copiloop/  \n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    assert gi.read_text(encoding="utf-8").count(".copiloop/") == 1


def test_ensure_gitignore_entry_does_not_treat_substring_as_match(
    tmp_path: Path,
) -> None:
    """``my.copiloop/`` (a longer line containing our pattern) should not block
    the append — only the exact stripped match counts."""
    gi = tmp_path / ".gitignore"
    gi.write_text("my.copiloop/\n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    content = gi.read_text(encoding="utf-8")
    # Both lines now present.
    assert "my.copiloop/" in content
    assert any(line.strip() == ".copiloop/" for line in content.splitlines())
...
def test_event_log_writer_flushes_after_each_write_via_subprocess_crash(
    ...
    script = textwrap.dedent(f"""
        ...
        from copiloop.persist import EventLogWriter
        from copiloop.events import make_event
...
def test_create_writers_aligns_filenames_across_artefacts(tmp_path: Path) -> None:
    ...
    stem so an operator can correlate by ``ls .copiloop/{logs,runs}/<stem>.*``."""
    bundle = create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
...
def test_create_writers_paths_under_dot_copiloop(tmp_path: Path) -> None:
    bundle = create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
    assert bundle.event_log.path.parent == tmp_path / ".copiloop" / "logs"
    assert bundle.run_summary.path.parent == tmp_path / ".copiloop" / "runs"
    assert bundle.diagnostics_path.parent == tmp_path / ".copiloop" / "logs"


def test_create_writers_no_dirs_until_first_write(tmp_path: Path) -> None:
    """Factory call alone does not create .copiloop/logs/ or .copiloop/runs/."""
    create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
    assert not (tmp_path / ".copiloop").exists()
...
def test_create_writers_gitignore_touch_is_idempotent(tmp_path: Path) -> None:
    """Running the factory twice does not duplicate the .copiloop/ line."""
    (tmp_path / ".gitignore").write_text("node_modules/\n", encoding="utf-8")
    create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
    create_writers(
        tmp_path,
...
def test_create_writers_diagnostics_log_lazy_mkdir(tmp_path: Path) -> None:
    """The diagnostic .log file (and .copiloop/logs/) are not created until
...
def test_writers_bundle_is_frozen() -> None:
    ...
    bundle = WritersBundle(
        ...
        diagnostics=logging.getLogger("copiloop.diagnostics.test_frozen_check"),
...
def test_persist_module_imports_are_constrained() -> None:
    """``persist.py`` may import only stdlib + ``copiloop.events``.
    ...
    of peer copiloop modules other than ``events`` — keeps persist as a
