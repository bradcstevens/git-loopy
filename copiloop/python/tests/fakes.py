"""Reusable test doubles (fakes) for the ``copiloop`` seams.
...
from copiloop.gate import GateResult, LoopFailure
from copiloop.gh import GhError, Issue, PullRequest, Repo
from copiloop.git import Commit, GitError


class FakeGitClient:
    """Stateful in-memory :class:`~copiloop.git.GitClient` for loop tests.
...
class FakeGitHubClient:
    """Stateful in-memory :class:`~copiloop.gh.GitHubClient` for source/loop tests.
    ...
    assertions, and injects per-method :exc:`~copiloop.gh.GhError` failures so a
    ...
    **Strike** progress or interpret close-keywords. That policy stays in
    :class:`copiloop.sources.GitHubIssueSource`, never in the client.
    ...
    **List and single-view are independently scriptable.** ``issue_list`` /
    ...
    ``*_view_errors`` / ``issue_close_errors`` inject a :exc:`~copiloop.gh.GhError`
    ...
    def issue_comment(self, number: int, comment: str) -> None:
        ...
        :exc:`~copiloop.gh.GhError` so a test drives the source's
...
class FakeGateRunner:
    """Scriptable in-memory :class:`~copiloop.gate.GateRunner` for Integration tests.
    ...
    Red results carry ``failure`` (a shared :class:`~copiloop.gate.LoopFailure`) so a
