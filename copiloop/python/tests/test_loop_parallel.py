...
:func:`copiloop.loop.run` seam with the SDK + git / gh / gate seams faked,
...
via the injected :class:`~copiloop.gate.GateRunner`, and closes the issue with
...
:class:`~copiloop.worktree.WorktreeSetup` (``COPILOOP_WORKTREE_SETUP`` or a
...
from copiloop.worktree import SetupResult
...
class _SpyWorktreeSetup:
    """A scripted :class:`~copiloop.worktree.WorktreeSetup` for the Wave e2e.
    ...
    **before** any Lane session starts: sessions are created in the concurrent
    ...
    :class:`~copiloop.worktree.SetupResult` so a test can drive the green path
...
def _diag_log(tmp_path: Path) -> str:
    """The run's human-readable diagnostics log (``.copiloop/logs/<...>.log``)."""
    logs_dir = tmp_path / ".copiloop" / "logs"
...
def test_make_worktree_setup_binds_env_command(tmp_path, monkeypatch) -> None:
    ...
    Proves the env-only knob (like ``COPILOOP_SEND_TIMEOUT_SECONDS``) reaches the
