"""Tests for ``copiloop.sinks`` (issue #22 — event sink fan-out).
...
:class:`~copiloop.sinks.SinkFanout` instead of calling the line-printer
:class:`~copiloop.ui.renderer.Renderer` directly. JSONL logging stays
...
from copiloop.ui import Renderer, RunSummary
from copiloop.ui import summary as summary_module
...
def test_sinks_module_imports_are_constrained() -> None:
    ...
    No ``rich`` / ``textual`` / ``copilot`` SDK / shell-side copiloop
