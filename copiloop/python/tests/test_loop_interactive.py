"""Tests for the interactive **driver seam** in :func:`copiloop.loop.run`.

Issue #23 / ADR-0001: when a driver is supplied, ``run`` registers the driver's
Textual-agnostic ``state`` (a :class:`~copiloop.interactive.state.LiveRunState`)
...
def _wire_empty_pool_repo(tmp_path: Any, monkeypatch: Any) -> _FakeClient:
    ...
    (tmp_path / "copiloop" / "prompt.md").write_text("ralph prompt\n", encoding="utf-8")
