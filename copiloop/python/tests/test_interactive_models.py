"""Tests for ``copiloop.interactive.models`` (issue #24 — picker row model).

The picker's row content, ordering, cursor default, effort logic, and cell
formatting live in a **deep + pure** module (stdlib + :mod:`copiloop.config`
...
* the projection from duck-typed SDK ``ModelInfo`` objects to
  :class:`~copiloop.interactive.models.ModelChoice` rows;
* policy-disabled models becoming non-selectable;
* the per-model supported-effort filter (intersected with the kit's
  sendable :data:`~copiloop.config.REASONING_EFFORTS`) and the auto-skip
...
from copiloop.interactive import models as models_module
...
def test_models_module_imports_are_constrained() -> None:
    """``models.py`` is deep + pure: stdlib + ``copiloop.config`` only.
    ...
    allow = {"__future__", "dataclasses", "typing", "copiloop.config"}
    seen: set[str] = set()
