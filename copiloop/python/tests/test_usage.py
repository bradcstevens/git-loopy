"""Tests for ``copiloop.usage`` (issue #39 — the ``UsageTally`` value object).
...
* The module imports only stdlib + ``copiloop.pricing`` (enforced via AST).
"""
...
from copiloop import usage as usage_module
from copiloop.pricing import ModelPricing, Pricing, estimate_cost
...
# ---------------------------------------------------------------------------
# Module purity — stdlib + copiloop.pricing only (enforced structurally)
# ---------------------------------------------------------------------------


def test_usage_module_imports_only_stdlib_and_pricing() -> None:
    """``usage.py`` MUST import only stdlib + ``copiloop.pricing``.
    ...
    and it reaches pricing through ``copiloop.pricing`` (itself stdlib-only),
