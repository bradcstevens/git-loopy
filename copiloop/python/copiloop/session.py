"""``copiloop.session`` — per-iteration SDK Session orchestrator.
...
``copiloop`` invocation) and owned by ``copiloop.loop`` (issue #10),
...
:class:`~copiloop.sinks.SinkFanout` for the duration of the iteration;
...
* **No coupling to peer modules.** The session module knows about the
  ...
  the sinks module (for the :class:`~copiloop.sinks.SinkFanout`
  fan-out target), and the emit module (for the shared
  :class:`~copiloop.emit.EventEmitter` fan-out seam). It explicitly does
  **not** import
  ``copiloop.gh`` / ``copiloop.git`` / ``copiloop.loop`` / ``copiloop.cli``
  / ``copiloop.config`` / ``copiloop.wrapper`` / ``copiloop.pricing``.
  Enforced by ``tests/test_session.py::test_session_module_imports_are_constrained``.
* **CopilotClient is not constructed here.** ``copiloop.loop`` owns the
...
* **``_record`` scrubs once, via the shared emitter.** :meth:`_record`
  delegates to
  :meth:`EventEmitter.dispatch <copiloop.emit.EventEmitter.dispatch>`
...
# skill-detection uses the same literal (``copiloop.ui.renderer``).
...
class SessionConfig(Protocol):
    ...
    :class:`copiloop.config.RunConfig` (issue #10) satisfies this
...
def _scrub_permission_args(args: Any, tool_name: str) -> Any:
    ...
    :func:`copiloop.events.scrub` only applies the load-bearing
...
def _request_identity(req: PermissionRequest) -> tuple[str, Any, str | None]:
    ...
    **Known degradation:** ``--deny-tool``/``COPILOOP_DENY_TOOLS`` entries
...
class IterationSession:
    ...
    loop slice (issue #10) constructs it once per ``copiloop``
    ...
    Attributes:
        ...
        config: A :class:`SessionConfig`-conforming object (typically
            ``copiloop.config.RunConfig``).
        event_log: The :class:`EventLogWriter` for replay-grade JSONL.
        sinks: The caller's :class:`~copiloop.sinks.SinkFanout`. Mapped SDK
        ...
        model: Optional model override; forwarded to the SDK. A bare base
            ...
            :mod:`copiloop.cli` strips any ``-<effort>`` suffix before
            this point).
        reasoning_effort: Optional reasoning-effort override forwarded to
            ...
            service then applies its own default. :mod:`copiloop.cli`
    ...
    def _record(self, envelope: dict[str, Any]) -> None:
        ...
        Delegates to the shared :class:`~copiloop.emit.EventEmitter`
