"""``copiloop.gate`` — the runner-side Integration gate seam (ADR-0009).
...
Like ``copiloop.git`` / ``copiloop.gh``, the gate is a **real seam**: callers
...
scripted ``tests.fakes.FakeGateRunner`` — through the ``copiloop.loop._make_gate_runner``
