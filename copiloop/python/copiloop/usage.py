"""``copiloop.usage`` — the ``UsageTally`` **Consumption** value object.
...
em-dash guard around :func:`~copiloop.pricing.estimate_cost` were, until this
...
* **Deep and pure.** Imports stay stdlib + :mod:`copiloop.pricing` (itself
...
* **Unknown-model semantics.** :meth:`UsageTally.cost` returns ``None`` (not
  ...
  matching :func:`~copiloop.pricing.estimate_cost` so callers render ``—``.
"""
...
from copiloop.pricing import Pricing, estimate_cost
...
class UsageTally:
    ...
    def cost(self, pricing: Pricing) -> Decimal | None:
        ...
        :func:`~copiloop.pricing.estimate_cost`, which itself returns ``None``
