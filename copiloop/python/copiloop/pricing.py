"""``copiloop.pricing`` — provider-list-price cost estimation and context utilisation.
...
* :func:`load_pricing` — resolve explicit path → ``COPILOOP_PRICING_FILE`` env var →
...
_ENV_OVERRIDE = "COPILOOP_PRICING_FILE"
...
def _packaged_path() -> Path:
    ...
    """
    return Path(str(files("copiloop") / "pricing.toml"))
...
def load_pricing(path: Path | None = None) -> Pricing:
    ...
    2. The ``COPILOOP_PRICING_FILE`` environment variable, if set and non-empty.
    3. The packaged ``copiloop/pricing.toml``.
