"""``copiloop.emit`` — the ``EventEmitter`` scrub-and-fan-out seam (issue #43).
...
per-**Iteration** :class:`~copiloop.session.IterationSession` each ran their own
copy of the pipeline — *compose -> write the replay JSONL -> fan out to the sink
list* — and the two copies disagreed:

* :meth:`IterationSession._record <copiloop.session.IterationSession._record>`
  scrubs the envelope **once**, then hands the *scrubbed* dict to both the
  :class:`~copiloop.persist.EventLogWriter` and the
  :class:`~copiloop.sinks.SinkFanout`. Correct.
* ``_Loop._emit`` (``copiloop/loop.py``) writes via the ``EventLogWriter`` —
  ...
  (``copiloop/sinks.py``). The replay log is clean; the live Renderer /
...
* **Deep and pure.** Imports stay ``__future__`` + :mod:`typing` +
  :mod:`copiloop.events` only — no ``rich`` / ``textual`` / SDK / ``persist`` /
...
from copiloop.events import make_event, scrub
...
class _EventLog(Protocol):
    ...
    :class:`~copiloop.persist.EventLogWriter` satisfies this structurally.
    """
...
class _Sinks(Protocol):
    ...
    :class:`~copiloop.sinks.SinkFanout` satisfies this structurally.
    """
...
class EventEmitter:
    ...
    exposing ``render(dict)`` (the :class:`~copiloop.sinks.SinkFanout`), and
    ...
    def emit(
        ...
        **payload: Any,
        ...
        Composes via :func:`copiloop.events.make_event` (``iter=iter_num``) and
    ...
    def dispatch(self, envelope: dict[str, Any]) -> None:
        ...
        The single :func:`copiloop.events.scrub` call runs *before* the guards,
