"""``copiloop.config`` — frozen per-invocation configuration.

The :class:`RunConfig` dataclass is the single load-bearing config seam
between :mod:`copiloop.cli` (which composes it from CLI flags + env
vars + defaults) and :mod:`copiloop.loop` (which consumes it).

It also satisfies — structurally, via Python's :pep:`544` Protocol
machinery — the :class:`copiloop.session.SessionConfig` Protocol, so
the loop can pass the same object to :class:`~copiloop.session.IterationSession`
...
* **No I/O at construction time.** ``pricing_file`` is a :class:`Path`
  reference — actually opening it is :func:`copiloop.pricing.load_pricing`'s
  job and only happens inside :func:`copiloop.loop.run`.
...
#: ``reasoningEffort`` is sent for such a model, so :mod:`copiloop.cli`
...
#: :data:`MODEL_REASONING_EFFORTS`). :mod:`copiloop.cli` uses this to
...
#: knob, used by :mod:`copiloop.cli`'s suffix-derivation helper and
...
class RunConfig:
    """Frozen per-invocation configuration for the ``copiloop`` runner.
    ...
    Attributes:
        ...
        reasoning_effort: Optional reasoning-effort override forwarded to
            ...
            every effort — some accept none at all — so :mod:`copiloop.cli`
        ...
        otel_enabled: ``True`` when OpenTelemetry tracing is enabled
            (either ``COPILOOP_OTEL_ENABLED=1`` or
        ...
        pricing_file: Optional explicit path to a ``pricing.toml``.
            ``None`` lets :func:`copiloop.pricing.load_pricing` resolve
            from ``COPILOOP_PRICING_FILE`` or the packaged default.
        parallel: Opt-in **Parallel mode** cap (ADR-0008). ``1`` (the
            default) is serial — :func:`copiloop.loop.run` drives the
