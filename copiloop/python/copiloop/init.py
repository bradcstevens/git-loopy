"""``copiloop.init`` — the first-run setup wizard (issue #53, ADR-0006/0007).

``copiloop init`` writes persisted **Config** (and, default-yes, an editable
``PROMPT.md`` override plus copiloop's agent skills) into a chosen **scope**
...
Design (mirrors :mod:`copiloop.settings` being the pure I/O half):
...
* **SDK-free until it fetches.** The model list is the only thing that touches
  the SDK, and only on the interactive path; ``copiloop init --yes`` uses the
  built-in default model / effort and never imports the SDK. The model rows reuse
  :func:`copiloop.interactive.models.to_model_choices` (stdlib + config only, no
...
from copiloop.config import MODEL_REASONING_EFFORTS
from copiloop.interactive.models import (
    ModelChoice,
...
def _resolve_targets(scope: str, repo_root: Path | None, env: Mapping[str, str]) -> _Targets:
    ...
    * **project** — ``<repo>/copiloop/config.toml``, ``<repo>/copiloop/PROMPT.md``,
      ``<repo>/.copilot/skills/``.
    * **global** — ``$XDG_CONFIG_HOME/copiloop/{config.toml,PROMPT.md}`` (else
      ``~/.config/copiloop/...``) and ``~/.copilot/skills/`` (Copilot's skills
...
def _static_choices() -> list[ModelChoice]:
    ...
    ``copiloop init`` still seeds a model without a backend.
    """
...
def _packaged_skills_path() -> Path:
    """copiloop's agent skills shipped inside the wheel (scaffolded by ``init``)."""
    return Path(str(files("copiloop") / "skills"))
...
def _scaffold_skills(skills_dir: Path, source: Path) -> None:
    """Copy copiloop's packaged agent skills into the scope's ``.copilot/skills``."""
    ...
    *,
    ...
    if scope is None:
        if assume_yes:
            ...
            labels = [
                "project  (this repository: <repo>/copiloop/)"
                if repo_root is not None
                else "project  (unavailable: not in a git repository)",
                "global   (this machine: ~/.config/copiloop/)",
            ]
            default_index = 0 if repo_root is not None else 1
            index = _ask_index(
                input_fn,
                output_fn,
                "Configure copiloop for which scope?",
                labels,
...
def run_init(
    *,
    ...
    """
    from copiloop.cli import _DEFAULT_MODEL, _DEFAULT_REASONING_EFFORT, _warn
    ...
    try:
        ...
        output_fn("copiloop init cancelled; nothing was written.")
        return 1
    ...
    try:
        if assume_yes:
            ...
            scaffold = _ask_yes_no(
                input_fn,
                "Also scaffold an editable PROMPT.md override and copiloop's "
    ...
    output_fn(
        f"copiloop is configured ({resolved_scope} scope). "
        "Run `copiloop` to start the loop."
    )
    return 0
