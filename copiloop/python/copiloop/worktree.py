"""``copiloop.worktree`` — the per-Lane worktree setup seam (#65, ADR-0008).
...
Like :mod:`copiloop.gate`, the setup is a **real, injectable seam**: callers hold
...
the ``copiloop.loop._make_worktree_setup`` factory, exactly like the git / gh /
gate factories.
...
* **Shell execution.** Commands run through the shell (``shell=True``) exactly as
  an operator would type them — the same trust model as :mod:`copiloop.gate`
