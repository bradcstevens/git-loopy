"""``copiloop.gh`` — typed subprocess wrapper around the ``gh`` CLI.

This module is the **only** place in ``copiloop/`` that talks to GitHub.
...
GitHub is a **real seam** (mirroring :class:`copiloop.git.GitClient`, #46):
:class:`copiloop.sources.GitHubIssueSource` holds a :class:`GitHubClient` (an
...
class GitHubClient(Protocol):
    ...
    Stateless: unlike :class:`copiloop.git.GitClient` there is **no cwd
    binding** — ``gh`` runs in the process cwd — so the methods keep their
    natural signatures. :class:`copiloop.sources.GitHubIssueSource` holds one
...
class SubprocessGitHubClient:
    ...
    :class:`copiloop.git.SubprocessGitClient` there is nothing to bind at
    ...
    def pr_list(self, label: str, state: str = "open") -> list[PullRequest]:
        ...
        :class:`copiloop.sources.GitHubIssueSource`).
