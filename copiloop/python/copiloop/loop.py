"""``copiloop.loop`` — async iteration driver for the AFK runner.

This is the orchestrator that ties every previously-merged module
together into a working ``copiloop`` invocation. It owns:

* The long-running :class:`copilot.CopilotClient` (one per ``copiloop``
  invocation; reused across iterations).
* The per-run :class:`~copiloop.persist.WritersBundle`
  (:class:`~copiloop.persist.EventLogWriter`,
  :class:`~copiloop.persist.RunSummaryWriter`, and the diagnostics
  logger).
* The per-run :class:`~copiloop.ui.RunSummary` and
  :class:`~copiloop.ui.Renderer`.
* The :class:`~copiloop.wrapper.NMTStrikeStateMachine`.
* The :class:`~copiloop.sources.IssueSource` — the per-invocation
...
The per-iteration :class:`~copiloop.session.IterationSession` is opened
...
6. Open :class:`~copiloop.session.IterationSession`,
...
9. **Completion backstop** via
   ...
   :class:`~copiloop.sources.Completion` produces one
...
13. Emit ``wrapper.iteration.end`` (renderer closes snapshot panel) and
    persist :class:`~copiloop.persist.IterationCounters` from the
    closed snapshot.
...
* **Inter-module fan-out via the shared ``EventEmitter``.** Every
  ...
  :class:`~copiloop.emit.EventEmitter` (issue #45). The emitter:
  1. Constructs an envelope via :func:`copiloop.events.make_event`.
  2. Scrubs it **once**, then writes that scrubbed dict as the JSONL line via
     ...
     registered.
  3. Hands the *same scrubbed* dict to the :class:`~copiloop.sinks.SinkFanout`,
     ...
     line-printer :class:`~copiloop.ui.renderer.Renderer`, which drives the
     Rich terminal output and RunSummary accumulator updates; the same fan-out
     is handed to each :class:`~copiloop.session.IterationSession` so SDK
...
from copiloop.persist import (
    IterationCounters,
    WritersBundle,
    create_writers,
)
from copiloop.pricing import Pricing, PricingError, load_pricing
from copiloop.session import IterationSession
from copiloop.sinks import EventSink, SinkFanout
from copiloop.sources import (
    AfkReadyItem,
    GitHubIssueSource,
    IssueSource,
    PrdsIssueSource,
)
from copiloop.telemetry import otel as telemetry
from copiloop.ui import Renderer, RunSummary, get_console
from copiloop.wrapper import (
...
def _make_client() -> CopilotClient:
    ...
    it (``monkeypatch.setattr("copiloop.loop._make_client", ...)``) to
...
def _make_git_client() -> git_module.SubprocessGitClient:
    ...
    (``monkeypatch.setattr("copiloop.loop._make_git_client", ...)``) to inject
    ...
    :class:`~copiloop.git.SubprocessGitClient` discovered from the process cwd:
    it resolves the repository root once (``git rev-parse --show-toplevel``) and
    binds every subsequent git call to it.

    Returns the concrete :class:`~copiloop.git.SubprocessGitClient` rather than
    the :class:`~copiloop.git.GitClient` protocol so :func:`run` can read
...
def _make_github_client() -> gh_module.SubprocessGitHubClient:
    ...
    — so tests can monkeypatch it
    (``monkeypatch.setattr("copiloop.loop._make_github_client", ...)``) to inject
    ...
    :class:`~copiloop.gh.SubprocessGitHubClient`.
...
def _make_gate_runner() -> gate_module.AgentsMdGateRunner:
    ...
    (``monkeypatch.setattr("copiloop.loop._make_gate_runner", ...)``) to a scripted
    ``tests.fakes.FakeGateRunner``. Production callers get a
    :class:`~copiloop.gate.AgentsMdGateRunner`, which runs a worktree's ``AGENTS.md``
...
def _make_worktree_setup() -> worktree_module.WorktreeSetup:
    ...
    (``monkeypatch.setattr("copiloop.loop._make_worktree_setup", ...)``) to a
...
class _Loop:
    """Stateful orchestrator for one ``copiloop`` invocation.
    ...
    def _emit(
        ...
        **payload: Any,
        ...
        Delegates to the shared :class:`~copiloop.emit.EventEmitter` (issue
        #45): it composes the envelope via :func:`copiloop.events.make_event`,
        ...
        dict out to the :class:`~copiloop.sinks.SinkFanout` — so the on-screen
    ...
    async def _run_one_iteration(
        ...
        OTel span tree: opens ``copiloop.iteration`` for the entire body,
        with three children — ``copiloop.collect_issues`` around the
        pool discovery, ``copiloop.session`` around the SDK session
        lifecycle, and ``copiloop.enforce_closures`` around the
        ...
        """
        with telemetry.span(
            "copiloop.iteration", iter=iter_num
        ) as iteration_span:
            self._emit(
            ...
            with telemetry.span("copiloop.collect_issues"):
            ...
            with telemetry.span("copiloop.enforce_closures"):
    ...
    def _record_counters(self, iter_num: int) -> None:
        ...
        and translates it to an :class:`~copiloop.persist.IterationCounters`
    ...
    async def drive(self) -> int:
        ...
        try:
            try:
                ...
                exit_code = 1
                self._diag.error(
                    "copiloop iteration %d crashed: %s: %s",
...
class _Lane:
    ...
    :class:`~copiloop.sources.AfkReadyItem` it works, the branch cut for it,
    the worktree path, the root-bound child :class:`~copiloop.git.GitClient`
...
class _ParallelLoop:
    ...
    **Integration (#62, ADR-0009).** A Wave ends by landing its green Lanes on
    ...
    :class:`~copiloop.gate.GateRunner` as the load-bearing gate, and on green
    ...
    async def drive(self) -> int:
        ...
        try:
            try:
                ...
                exit_code = 1
                self._diag.error(
                    "copiloop parallel round %d crashed: %s: %s",
                    iter_num, type(exc).__name__, exc,
                )
                raise
        finally:
    ...
    def _setup_lane_worktree(self, lane: _Lane) -> None:
        ...
        Runs the injected :class:`~copiloop.worktree.WorktreeSetup` — the
    ...
    async def _integrate_wave(self, iter_num: int, lanes: list[_Lane]) -> int:
        ...
        1. Merge the Lane branch into base and re-run the full feedback loops from
           the *runner* side via the injected :class:`~copiloop.gate.GateRunner`.
        ...
        3. A merge **conflict** (:meth:`~copiloop.git.GitClient.abort_merge`) or a
           clean merge whose gate goes **red** or cannot run
           (:meth:`~copiloop.git.GitClient.revert_merge`) is undone so base stays
    ...
    def _gate_green(
        ...
        A **red** gate or a :exc:`~copiloop.gate.GateError` (cannot gate at all)
...
class InteractiveDriver(Protocol):
    ...
    :class:`copiloop.interactive.driver.InteractiveDriver`. It is referenced
    here only as a **structural Protocol** so :mod:`copiloop.loop` never
    ...
    * :attr:`state` is the Textual-agnostic
      :class:`~copiloop.interactive.state.LiveRunState`, registered by
    ...
    * :meth:`attach_detach` receives the exit-model handoff (issue #28): the
      swappable :class:`~copiloop.sinks.SinkFanout`, the parked line-printer
...
async def run(config: RunConfig, *, driver: InteractiveDriver | None = None) -> int:
    """Drive one ``copiloop`` invocation to completion.
    ...
    Args:
        ...
        driver: Optional interactive driver (ADR-0001 observer model). When
            ...
            :meth:`~copiloop.sinks.SinkFanout.set_sinks`.
    ...
    try:
        ...
        print(
            f"copiloop: failed to resolve git repository root: {exc}",
            file=sys.stderr,
        )
        return 1
    repo_root = git.root
    ...
    try:
        ...
        print(f"copiloop: pricing load failed: {exc}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(
            f"copiloop: failed to construct writers bundle: "
    ...
    try:
        ...
        print(
            f"copiloop: failed to construct CopilotClient: "
            f"{type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
    ...
    try:
        try:
            with writers.event_log, writers.run_summary:
                ...
                # W3C trace context — see copiloop.telemetry.otel
                # module docstring for the propagation contract.
                with telemetry.span("copiloop.run"):
                    try:
                        if driver is None:
            ...
            print(
                f"copiloop: writers __exit__ failed: "
                f"{type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
            exit_code = 1
        ...
        # Drain OTel exporters AFTER the root `copiloop.run` span has
