"""``copiloop.git`` — typed subprocess seam around the ``git`` CLI.

Every external ``git`` call in ``copiloop/`` flows through this module so
...
* :class:`Commit` — frozen value object carrying ``sha`` / ``subject`` /
  ...
  message (``subject + "\\n" + body``) so :func:`copiloop.wrapper.extract_close_refs`
...
class Commit:
    ...
    def message(self) -> str:
        ...
        This is what :func:`copiloop.wrapper.extract_close_refs` expects —
...
class SubprocessGitClient:
    ...
    def commits_between(self, pre: str, head: str) -> list[Commit]:
        ...
        :func:`copiloop.wrapper.extract_close_refs` against ``commit.message``.
