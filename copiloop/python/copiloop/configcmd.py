"""``copiloop.configcmd`` — the ``copiloop config`` subcommand group (issue #56).
...
Design (mirrors :mod:`copiloop.init`):
...
* **Effective values come from the resolver.** ``get`` / ``list`` reuse
  :func:`copiloop.cli.resolve_config` over a defaulted args namespace + the live
  ...
  **stdout**; warnings / errors to **stderr**, so ``$(copiloop config get model)``
  captures only the value.
* **SDK-free.** Imports only stdlib + :mod:`copiloop.settings`, and lazily
  :mod:`copiloop.cli` (already imported at dispatch). It never imports the loop /
  SDK / renderer, so ``copiloop config`` dispatch stays snappy.
"""
...
from copiloop.config import REASONING_EFFORTS

if TYPE_CHECKING:
    from copiloop.cli import ResolvedConfig
...
class ConfigCommandError(Exception):
    """A user-facing ``copiloop config`` failure (bad key/value, missing scope).

    Carries a clean, prefix-free message; the run_* wrappers render it to stderr
    with the kit's ``copiloop: error:`` prefix and return a non-zero exit code.
    """
...
def _display_value(value: object) -> str:
    ...
    tail, and a list as a comma-joined string (matching the ``COPILOOP_DENY_*``
    env spelling).
    """
    if value is None:
        return ""
...
def _resolve(
    ...
    *,
    ...
    Reuses :func:`copiloop.cli.resolve_config` over a fully-defaulted args
    namespace, the live ``env``, and both loaded scopes, so ``get`` / ``list``
    report what a bare ``copiloop`` would actually use. ``cli`` is imported
...
def run_path(
    *,
    ...
    if repo_root is not None:
        ...
        err(
            "copiloop: note: project scope unavailable (not in a git "
            "repository)."
        )
...
def run_edit(
    *,
    ...
    if not (editor and editor.strip()):
        err(
            "copiloop: error: no editor configured; set $VISUAL or $EDITOR "
