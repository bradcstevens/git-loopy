"""``copiloop.sources`` — abstract source of AFK-ready work items.
...
* :class:`GitHubIssueSource` — the default backend used when
  ...
  :func:`copiloop.wrapper.extract_close_refs` + :func:`gh.issue_close`.
* :class:`PrdsIssueSource` — the legacy local-markdown backend used
  ...
  prds/<feat>/done/`` per ``copiloop/PROMPT.md``. The wrapper has no
  PRDs-side completion backstop.

Design notes:

* **The IssueSource Protocol is the seam.** :mod:`copiloop.loop` holds
...
from copiloop.git import Commit
from copiloop.wrapper import extract_close_refs, filter_to_pool
...
def is_pr_afk_ready(pr: gh_module.PullRequest) -> bool:
    ...
    Args:
        pr: A :class:`copiloop.gh.PullRequest` (typically fetched via
            :func:`copiloop.gh.pr_view`, so its ``comments`` are populated).
...
class GitHubIssueSource:
    ...
    :func:`copiloop.wrapper.extract_close_refs` so the parser is shared
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            gh: The injected :class:`copiloop.gh.GitHubClient` seam — the raw
                GitHub mechanics (list / view / close). Production wiring injects
                a :class:`copiloop.gh.SubprocessGitHubClient` (via
                :func:`copiloop.loop._make_github_client`); the sources tests
    ...
    def preflight(self) -> int | None:
        ...
        try:
            ...
            self._diag.error(
                "gh repo view failed: %s. copiloop must be run from inside a "
    ...
    def _try_close_one(
        ...
        comment = (
            ...
            f"Closed by the copiloop loop because the agent did not run "
...
def _format_github_pr_block(pr: gh_module.PullRequest) -> str:
    ...
    ``copiloop/PROMPT.md`` (check out the branch, finish the diff, push, do
...
class PrdsIssueSource:
    ...
    * Sort the combined results by their repo-relative POSIX path so
    ...
    ``copiloop/PROMPT.md``'s local-markdown mode contract.
