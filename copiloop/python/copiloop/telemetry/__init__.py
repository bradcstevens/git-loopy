"""``copiloop.telemetry`` — opt-in OpenTelemetry tracing.
...
:mod:`copiloop.telemetry.otel`. See its docstring for the activation
...
the lazy imports inside :mod:`copiloop.telemetry.otel`. Operators who
install with ``uv sync`` (no ``--extra otel``) see exactly zero
``opentelemetry`` modules in ``sys.modules`` after a ``copiloop``
