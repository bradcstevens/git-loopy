"""``copiloop.telemetry.otel`` — the OpenTelemetry seam.

A **single switch** module that the rest of ``copiloop`` uses for span
...
* ``OTEL_EXPORTER_OTLP_ENDPOINT`` env var is set to a non-empty string —
  ...
  collector enable copiloop's tracing the same way they enable any other
  Python OTel client.
* ``COPILOOP_OTEL_ENABLED`` env var is set to a truthy value (``"1"``,
...
half-emitted span tree (e.g. a test resetting ``COPILOOP_OTEL_ENABLED``
between iterations of the same loop would otherwise leave the
``copiloop.run`` root span open while subsequent ``span()`` calls
...
The runner emits this tree::
    ...
    copiloop.run                  (root, per copiloop invocation)
...
SDK-emitted spans nest under ``copiloop.session`` because the Copilot
...
::
    ...
    with telemetry.span("copiloop.iteration", iter=iter_num) as sp:
        ...
...
:class:`ImportError`, logs a warning via the ``copiloop.telemetry``
...
_logger = logging.getLogger("copiloop.telemetry")
...
def _build_tracer() -> Any:
    ...
    * Otherwise, install a fresh :class:`TracerProvider`. If
      ...
      ``COPILOOP_OTEL_ENABLED=1`` is set, install a provider with no
    ...
    return trace.get_tracer("copiloop")
...
def _init() -> None:
    ...
    if enabled:
        try:
            ...
            _logger.warning(
                "OTel was requested (COPILOOP_OTEL_ENABLED or "
                "OTEL_EXPORTER_OTLP_ENDPOINT is set) but the "
                "[otel] extra is not installed: %s. "
                "Install with `uv sync --project copiloop/python --extra otel` "
                "(or `pip install 'copiloop[otel]'`) to enable tracing.",
                exc,
            )
            _state["tracer"] = None
...
def span(name: str, **attrs: Any) -> Iterator[Any]:
    ...
    Args:
        name: The span name. Convention: dotted, lowercase, prefixed with
            ``copiloop.`` (matches the contract in the module docstring).
    ...
    Example::
        ...
        with telemetry.span("copiloop.iteration", iter=1) as sp:
            ...
            sp.set_attribute("issue", first_ref)
    """
    _init()
...
def build_sdk_telemetry_config() -> Optional[dict[str, Any]]:
    ...
    * If only ``COPILOOP_OTEL_ENABLED=1`` is set (no endpoint), the dict
...
def force_flush(timeout_millis: int = 30_000) -> None:
    ...
    outer ``finally`` block (in :func:`copiloop.loop.run`) so that
...
def reset_for_tests() -> None:
    ...
    Intended for test fixtures that need to toggle ``COPILOOP_OTEL_ENABLED``
