"""``copiloop.interactive.driver`` — peer-task orchestration (ADR-0001).
...
:func:`copiloop.loop.run` holds this object structurally (its ``InteractiveDriver``
...
:mod:`copiloop.loop` never imports Textual.
"""
...
from copiloop.interactive.app import CopiloopApp
from copiloop.interactive.state import LiveRunState
...
#: ADR-0003).
AppFactory = Callable[..., "CopiloopApp"]
...
class InteractiveDriver:
    ...
    def __init__(
        ...
        state: LiveRunState,
        *,
        app_factory: AppFactory = CopiloopApp,
    ) -> None:
        self.state = state
        self._app_factory = app_factory
        #: Loop-owned panes attached by :func:`copiloop.loop.run` (issue #26)
        ...
        #: Exit-model handoff attached by :func:`copiloop.loop.run` (issue #28)
        #: before :meth:`run`: the swappable :class:`SinkFanout`, the parked
        #: line-printer :class:`~copiloop.ui.renderer.Renderer` to swap in on a
    ...
    def attach_panes(
        ...
        *,
        ...
        Called by :func:`copiloop.loop.run` after it constructs the shared
        :class:`~copiloop.ui.summary.RunSummary` and the buffer-backed capture
    ...
    def attach_detach(
        ...
        *,
        ...
        Called by :func:`copiloop.loop.run` on the interactive path with the
        run's swappable :class:`~copiloop.sinks.SinkFanout`, the parked
        line-printer :class:`~copiloop.ui.renderer.Renderer` (kept out of the
    ...
    async def run(self, drive: Callable[[], Coroutine[object, object, int]]) -> int:
        ...
        * natural completion / crash also leave a scrollback record (the table),
        ...
        loop_task: asyncio.Task[int] = asyncio.create_task(
            drive(), name="copiloop-loop"
        )
        app_task: asyncio.Task[None] = asyncio.create_task(
            app.run_async(), name="copiloop-tui"
        )
