"""``copiloop.interactive.state`` — the Textual-agnostic live run model.
...
:mod:`copiloop.sinks`). Enforced by
``tests/test_interactive_state.py::test_state_module_imports_are_constrained``.

Because importing :mod:`copiloop.events` would pull the Copilot SDK (it types
...
# than imported from ``copiloop.events``, which would pull the SDK) and kept in
...
# tail. Re-declared locally (importing ``copiloop.events`` would pull the SDK)
...
#: worked it. Re-declared locally (importing ``copiloop.events`` would pull the
...
class IssueLedgerEntry:
    ...
    * ``usage`` — per-issue **Consumption** (issue #36) as a shared
      :class:`~copiloop.usage.UsageTally` (issue #41): the input/output tokens
...
class LiveRunState:
    ...
    Satisfies the :class:`copiloop.sinks.EventSink` protocol structurally
    ...
    def __init__(
        ...
        *,
        ...
        #: :class:`~copiloop.usage.UsageTally` (issue #41) that ``usage.tokens``
...
class QueueRow:
    ...
    **Consumption** as the shared :class:`~copiloop.usage.UsageTally` (issue
...
def queue_rows(state: LiveRunState, *, now: float | None = None) -> list[QueueRow]:
    ...
    **Active** duration (``active_seconds``), which ticks against ``now``
    ...
    :class:`~copiloop.usage.UsageTally` passed straight through (``usage``) —
