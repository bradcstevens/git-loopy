"""``copiloop.interactive.picker`` — startup picker orchestration (issue #24).
...
the static :data:`~copiloop.config.MODEL_REASONING_EFFORTS` matrix +
env/default in :mod:`copiloop.cli`) is returned unchanged. Quitting the picker
...
Textual picker app (:mod:`copiloop.interactive.picker_app`) is imported lazily
...
backend (mirrors how :mod:`copiloop.interactive.driver` keeps Textual out of
:mod:`copiloop.loop`).
"""
...
#: default lazily runs the real :class:`~copiloop.interactive.picker_app.ModelPickerApp`.
...
async def _run_picker_app(
    ...
    from copiloop.interactive.picker_app import ModelPickerApp
...
async def resolve_run_model(
    ...
    *,
    ...
    Args:
        ...
        warn: Non-fatal warning sink (the kit's stderr ``copiloop: warning:``).
