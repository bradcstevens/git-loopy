"""``copiloop.interactive.detect`` — interactive-path gating (issue #23).

Decides whether one ``copiloop`` invocation takes the **interactive** path (a
...
2. The ``COPILOOP_INTERACTIVE`` env override (``1``/``true``/... vs ``0``/...).
...
``--no-interactive``, ``COPILOOP_INTERACTIVE=0``, or ``[tui]`` absent) yields
    ...
    *,
    ...
    Args:
        flag: Tri-state ``--interactive`` (``True``) / ``--no-interactive``
            (``False``) / neither (``None``).
        env_value: Raw ``COPILOOP_INTERACTIVE`` value (``None``/blank = unset).
    ...
    if not textual_importable:
        if explicit:
            warn(
                ...
                "printer. Install it with: pip install 'copiloop[tui]'"
            )
        return False
...
def resolve_model_selection(*, flag: bool | None, env_value: str | None) -> bool:
    ...
    2. The ``COPILOOP_MODEL_SELECT`` env override (``1``/``true``/... vs ``0``/...).
    3. Off (opt-in).

    Args:
        flag: Tri-state ``--select-model`` (``True``) / ``--no-select-model``
            (``False``) / neither (``None``).
        env_value: Raw ``COPILOOP_MODEL_SELECT`` value (``None``/blank = unset).
