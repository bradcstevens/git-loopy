"""``copiloop.interactive.models`` — the pure picker row model (issue #24).
...
**deep + pure** module: stdlib + :mod:`copiloop.config` only, **no Textual** and
...
(ADR-0001; mirrors :mod:`copiloop.interactive.state`). The Textual presentation
lives in :mod:`copiloop.interactive.picker_app`; the live fetch + fallback
orchestration in :mod:`copiloop.interactive.picker`.
...
``.default_reasoning_effort``. This mirrors how :mod:`copiloop.interactive.state`
...
class ModelChoice:
    ...
    Attributes:
        ...
        supported_efforts: The model's live supported reasoning efforts,
            **filtered** to the kit's sendable :data:`~copiloop.config.REASONING_EFFORTS`
