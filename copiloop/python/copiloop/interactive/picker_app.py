"""``copiloop.interactive.picker_app`` — the two-stage Textual picker (issue #24).
...
:mod:`copiloop.interactive.models` and returns the operator's
:class:`~copiloop.interactive.models.Selection` (or ``None`` if quit):
...
:mod:`copiloop.interactive.app` — it is reached only on the interactive path,
lazily, after :func:`copiloop.interactive.picker.fetch_live_models` succeeds.
The pure row model lives in :mod:`copiloop.interactive.models`; everything here
is presentation.
"""
...
#: tell which stage fired (mirrors how :mod:`copiloop.interactive.app` routes by
#: ``event.data_table.id``).
