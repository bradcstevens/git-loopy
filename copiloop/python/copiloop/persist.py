"""``copiloop.persist`` — filesystem side of observability.
...
==================  ==========================================  =================================
Event log           ``.copiloop/logs/<iso>-<run_id>.jsonl``        Replay-grade JSONL, one event
                                                                per line, append-only.
Run summary         ``.copiloop/runs/<iso>-<run_id>.json``         Per-iteration counter rollup,
                                                                emitted on close.
Process diag.       stderr + ``.copiloop/logs/<iso>-<run_id>.log``  Human-readable diagnostics;
                                                                stderr stream is primary, the
                                                                ``.log`` file is the mirror.
==================  ==========================================  =================================

All artefacts live under the **repo root** (resolved by callers via
:func:`copiloop.git.repo_root`). The directories are created lazily on
...
* :class:`EventLogWriter` — append-only JSONL writer, context manager.
  Every event is routed through :func:`copiloop.events.scrub` and
  :func:`copiloop.events.to_jsonl_line` before being flushed.
...
* :func:`create_writers` — the canonical factory. Constructs an
  ...
  appends ``.copiloop/`` to ``.gitignore`` if the file exists and the entry
...
* :func:`ensure_gitignore_entry` — exposed for tests; called by
  :func:`create_writers`.
* :data:`GITIGNORE_ENTRY` — the literal ``.copiloop/`` line.
...
* **Idempotent ``.gitignore`` touch.** :func:`ensure_gitignore_entry`
  matches the existing line by ``line.strip() in {".copiloop/", ".copiloop"}``
...
from copiloop.events import scrub, to_jsonl_line
...
def make_run_id(
    *,
    ...
    for ``ls .copiloop/logs/`` and grep-based log inspection.
...
def ensure_gitignore_entry(repo_root: Path) -> None:
    """Append ``.copiloop/`` to ``repo_root/.gitignore`` if the entry is missing.
    ...
    * ``.gitignore`` already contains a line whose ``.strip()`` value is
    ...
    for line in existing.splitlines():
        if line.strip() in {".copiloop/", ".copiloop"}:
            return
...
class EventLogWriter(AbstractContextManager["EventLogWriter"]):
    ...
    is routed through :func:`copiloop.events.scrub` and
    :func:`copiloop.events.to_jsonl_line` (which scrubs again — the
    ...
    def write(self, event: dict[str, Any]) -> None:
        ...
        Args:
            event: Event dict (typically from :func:`copiloop.events.make_event`).
        """
        if self._fh is None:
...
def create_writers(
    ...
    *,
    ...
    """Construct an aligned :class:`WritersBundle` for one ``copiloop`` invocation.

    Side effects:

    * Calls :func:`ensure_gitignore_entry` against ``repo_root`` — adds
      ``.copiloop/`` to ``.gitignore`` if the file exists and the entry is
    ...
    * Configures a named :class:`logging.Logger` for diagnostics
    ...
    Does **not** create any directories or files under ``.copiloop/`` —
    ...
    logs_dir = repo_root / ".copiloop" / "logs"
    runs_dir = repo_root / ".copiloop" / "runs"
...
def _format_rfc3339_ms(dt: datetime) -> str:
    ...
    Matches the JSONL envelope's ``ts`` format from :mod:`copiloop.events`
...
def _build_diagnostics_logger(run_id: str, log_path: Path) -> logging.Logger:
    ...
    first-call's path.
    """
    logger = logging.getLogger(f"copiloop.diagnostics.{run_id}")
