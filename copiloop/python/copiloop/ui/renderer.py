"""``copiloop.ui.renderer`` — event-driven terminal renderer.
...
:func:`copiloop.events.map_sdk_event` so each "print" the renderer
...
class Renderer:
    ...
    def _on_run_start(self, event: dict[str, Any]) -> None:
        ...
        text.append("copiloop run started", style=STYLES["panel_title"])
        if run_id:
...
# ---------------------------------------------------------------------------
# Dispatch table — keyed on event-type literals from ``copiloop.events``
# ---------------------------------------------------------------------------
