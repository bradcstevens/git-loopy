"""``copiloop.ui.summary`` — per-iteration counter accumulator + frozen UI artefacts.
...
translate iteration snapshots into :class:`copiloop.persist.IterationCounters`
...
* **Two-dataclass posture.** :class:`IterationSnapshot` (UI) and
  :class:`copiloop.persist.IterationCounters` (persist) intentionally
...
* **First non-None model wins — via the shared UsageTally.** Some SDK
  ...
  unknown-model cost guard) is now the :class:`~copiloop.usage.UsageTally`'s
...
* **context_used = tokens_in + tokens_out.** Read straight off the tally
  (:attr:`~copiloop.usage.UsageTally.total_tokens`); matches the schema
  example in :mod:`copiloop.persist`. Labelled as "observed tokens" in the
...
from copiloop.pricing import Pricing, context_utilisation
...
class IterationSnapshot:
    ...
    :class:`~copiloop.usage.UsageTally`; ``model`` / ``tokens_in`` /
    ...
    def context_used(self) -> int:
        ...
        :attr:`~copiloop.usage.UsageTally.total_tokens`). Labelled
    ...
    def cost_usd(self, pricing: Pricing) -> Optional[Decimal]:
        ...
        Delegates to :meth:`~copiloop.usage.UsageTally.cost`, which carries
    ...
    def to_counters_kwargs(self, *, pricing: Pricing) -> dict:
        ...
        :class:`copiloop.persist.IterationCounters`.

        Returning a dict (rather than an :class:`IterationCounters`
        instance directly) keeps this UI module's import graph free of
        ``copiloop.persist``. The loop slice (#10) does::

            from copiloop.persist import IterationCounters
        ...
        :class:`copiloop.persist.IterationCounters` ever gains a new
...
class RunSummary:
    ...
    :class:`~copiloop.ui.renderer.Renderer`. The renderer subscribes to
    ...
    Attributes:
        pricing: :class:`copiloop.pricing.Pricing` table used for cost
