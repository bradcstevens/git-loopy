...
4. Run `/setup-matt-pocock-skills` to populate the trailing **Agent skills** block. It writes a per-repo `docs/agents/` triplet too.
...
`<ONE_OR_TWO_SENTENCE_PROJECT_DESCRIPTION>`. See [`BRIEF.md`](BRIEF.md) for the full problem statement and the canonical PRD at `<PRD_LOCATION>` (e.g. `docs/PRD.md` or a GitHub issue URL).
...
`ralph/sh-afk.sh` is an unattended Copilot CLI loop that pulls AFK-ready issues (`ready-for-agent` label, body contains `## Parent` plus `## Acceptance criteria`), feeds each one to a fresh `copilot --yolo -p` invocation against [`ralph/PROMPT.md`](ralph/PROMPT.md), and **terminates cleanly when the AFK-ready pool is empty** or **aborts non-zero after `MAX_NMT_STRIKES` (default 3) consecutive no-progress iterations**. After each iteration the wrapper backstops the agent's `gh issue close` step by walking new commits for `Closes/Fixes/Resolves #N` references restricted to that iteration's AFK-ready pool — if the agent forgot to call `gh issue close`, the wrapper does it. The wrapper also refuses to start a new iteration if the working tree is dirty. See the header comment in [`ralph/sh-afk.sh`](ralph/sh-afk.sh) for env-var and usage details. The companion prompt routes work to `/diagnose`, `/prototype`, `/tdd`, `/improve-codebase-architecture`, `/zoom-out`, and `/grill-with-docs` at the appropriate phase.

A Python peer variant lives at [`ralph/python/`](ralph/python/) and implements the **same** wrapper contract (same `ready-for-agent` filter, same `## Parent` + `## Acceptance criteria` discriminator, same `Closes/Fixes/Resolves #N` auto-close backstop, same `MODEL` / `ISSUE_SOURCE` / `MAX_NMT_STRIKES` env vars, same exit conditions) on the GitHub Copilot Python SDK. It adds a richer terminal UX (frozen iteration `Panel`s, per-iteration token + estimated-cost signal, `.ralph/logs/*.jsonl` replay log, `.ralph/runs/*.json` per-iteration rollup, opt-in OpenTelemetry tracing) at the cost of a one-time `uv sync --project ralph/python` bootstrap. See [`ralph/python/README.md`](ralph/python/README.md) for bootstrap, invocation, exit codes, env-var surface, and observability artefact locations. The commit-message contract below applies to both runners verbatim.
...
> 📝 The three subsections below are owned by `/setup-matt-pocock-skills`. Run that skill in a fresh repo to populate them from real choices (GitHub vs GitLab vs local-markdown, label vocabulary, single- vs multi-context). The skill also writes the matching `docs/agents/*.md` triplet that each subsection points to.
