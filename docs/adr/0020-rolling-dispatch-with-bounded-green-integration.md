...
its design branch. [PRD #219](https://github.com/bradcstevens/git-loopy/issues/219) and the
design branch still refer to it by its authored number.
...
- The scheduler, source seam, contribution ledger, Event schema, replay reducers,
  ...
  changes.
- The Rolling dispatch and Lane contribution glossary update this decision work confirmed
  has since landed in `CONTEXT.md` on `main`, which is where the vocabulary below is
  defined. **Wave** remains historical ADR-0008 vocabulary.
