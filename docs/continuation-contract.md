...
Python and PowerShell additionally advertise their trusted immutable-revision protocol and explicit
`repair-index`; shell remains on the atomic-record capability subset. Mode is `off`.
...
Python and PowerShell immutable-revision Reconciliation discover issues and pull requests with a
...
Python and PowerShell Reconciliation return an opaque `sha256:` observation token over the
...
Callers select the Python or PowerShell immutable-revision capability with `revision_protocol: true`
on `reconcile`, then pass the exact `observation` and ordered `parents` to `publish`. Omitting those
...
Normal Reconciliation reports missing or stale index labels but never mutates them. Python and
PowerShell `repair-index` are the only index mutation paths. Each command authenticates the operator
and every record author, adds labels to trusted carriers, and removes labels only from artifacts with
no marked record. Publication still establishes the label before append and rereads the exact comment before
