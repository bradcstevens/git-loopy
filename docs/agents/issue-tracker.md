...
- **Planning documents**: Titles beginning with `PRD:` or `Spec:` (case-insensitive),
  and issues labelled `wayfinder:map`, are reference documents, not work for git-loopy.
  Never label them `ready-for-agent`; remove that label if present, without closing the
  document or changing other labels.
