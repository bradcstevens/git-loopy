...
Contract 2.9 includes the staged migration, affected-work refusal and shared
preflight-deadline obligations below, alongside Route publication (§14.5) and
Routing preparation (§14.6). Their existing Event vocabulary is pinned in
`event-schema.json`; the Event wire compatibility axis remains 1.2. This
declaration correction adds no Event fields, activates no Dynamic defaults,
and leaves historical streams' interpretation unchanged.
