...
Your issue is already **AFK-ready** — the runner filtered to issues that carry the `ready-for-agent` label and have a `## What to build` plus `## Acceptance criteria` section (a `## Parent` section is optional), put them in order, and bound the head of that order to this iteration. Do not pick up anything else. Issues whose titles begin with `PRD:` or `Spec:` (case-insensitive), or that carry the `wayfinder:map` label, are planning documents: never work on them or label them `ready-for-agent`. Work their implementation tickets instead. A PR block is AFK-ready by a different test — it carries an `## Agent Brief` (in the PR body or a comment); follow that brief.
