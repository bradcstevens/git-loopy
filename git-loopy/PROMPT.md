...
- If your issue turns out to be unworkable end to end (already done, or blocked on something you cannot satisfy from here), say so in a `gh issue comment` on that issue, then output `<promise>NO MORE TASKS</promise>` and stop. If what's blocking you is a genuine open blocker — a real, currently-open issue you actually identified, not a guess, a preference, or something you merely suspect exists — and `docs/agents/issue-tracker.md` says the tracker in use offers a dependency affordance (a way to record one issue as blocked on another natively, which local-markdown `prds` mode does not), also record that blocking relationship as a native tracker dependency of your issue on the blocking issue, in addition to the comment. Never record a dependency for a blocker you have not actually confirmed is a real open issue, and never invent one to satisfy this rule — a blocker that isn't an issue at all still only gets the comment, then NMT, exactly as before. Never substitute a different issue — the runner decides what comes next. The wrapper tolerates NMT only if no work was done; if you repeatedly emit it while AFK-ready issues remain, the wrapper will abort with a non-zero exit so a human can investigate.
