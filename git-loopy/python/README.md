...
tracker access does not renew its budget. A materially changed final assignment
gets a new budget. Neither state blocks locally recorded work. A failed local
final-resolution record is different and starts no work.
An exhausted label replacement may leave a stale Route label, or no Route label
...
stale/missing associations after replacement failure. Capability withdrawal
