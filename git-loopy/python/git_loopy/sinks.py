"""``git_loopy.sinks`` — event sink protocol + swappable fan-out (issue #22).

The AFK loop and its per-iteration
:class:`~git_loopy.session.IterationSession` no longer hand events to the
line-printer :class:`~git_loopy.ui.renderer.Renderer` directly. They dispatch
...
class EventSink(Protocol):
    ...
    The line-printer :class:`~git_loopy.ui.renderer.Renderer` satisfies this
    ...
    **not** JSONL artefacts — the replay log carries only the final, scrubbed
    ...
    :class:`~git_loopy.interactive.state.LiveRunState`, the line-printer
    :class:`~git_loopy.ui.renderer.Renderer`) additionally accepts a trailing
