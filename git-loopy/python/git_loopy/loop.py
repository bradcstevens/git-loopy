    ...
    async def _drive_rolling(self) -> tuple[str, int, int]:
        ...
        try:
            while True:
                ...
                # While the Pin's Lane is in setup, refill waits behind it
                # (#645), so the serial peek waits too: latching serial demand
                # now would stop the refill the Pin's binding releases, and
                # the Lanes behind the Pin would lose their turn to it.
                serial_pool_seen = (
                    False
                    if scheduler.lane_first_in_setup
