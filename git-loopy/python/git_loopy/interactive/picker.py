"""``git_loopy.interactive.picker`` — startup picker orchestration (issue #24).
...
the static :data:`~git_loopy.config.MODEL_REASONING_EFFORTS` matrix +
env/default in :mod:`git_loopy.cli`) is returned unchanged. Quitting the picker
...
async def _run_picker_app(
    ...
    from git_loopy.interactive.picker_app import ModelPickerApp
