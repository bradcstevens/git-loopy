"""``git_loopy.interactive.models`` — the pure picker row model (issue #24).
...
**deep + pure** module: stdlib + :mod:`git_loopy.config` only, **no Textual** and
...
(ADR-0001; mirrors :mod:`git_loopy.interactive.state`). The Textual presentation
lives in :mod:`git_loopy.interactive.picker_app`; the live fetch + fallback
orchestration in :mod:`git_loopy.interactive.picker`.
...
``.default_reasoning_effort``. This mirrors how :mod:`git_loopy.interactive.state`
...
class ModelChoice:
    ...
    Attributes:
        ...
        supported_efforts: The model's live supported reasoning efforts,
            **filtered** to the kit's sendable :data:`~git_loopy.config.REASONING_EFFORTS`
