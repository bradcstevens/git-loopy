...
Tell the user the setup is complete: the `git-loopy/` AFK tooling was cloned into the repo (or already present), and which engineering skills will now read from the `docs/agents/*.md` files. Mention they can edit `docs/agents/*.md` directly later — re-running this skill is only necessary if they want to switch issue trackers, re-fetch the `git-loopy/` tooling (remove `git-loopy/` first), or restart from scratch.
