...
def read_run_required_vocabulary(repo_root: Path | None) -> tuple[LabelSpec, ...]:
    ...
    ``wayfinder:map`` only to keep a map out of the **Pool**, which an absent
    label satisfies vacuously. Failing preflight on one would stop a loop over
    a label the loop does not require.
