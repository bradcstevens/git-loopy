...
==================  ==========================================  =================================
Event log           ``.git-loopy/logs/<iso>-<run_id>.jsonl``        Replay-grade JSONL, one event
                                                                per line, append-only.
Run summary         ``.git-loopy/runs/<iso>-<run_id>.json``         Per-iteration counter rollup,
                                                                emitted on close.
...
* :class:`EventLogWriter` — append-only JSONL writer, context manager.
  Every event is routed through :func:`git_loopy.events.scrub` and
  :func:`git_loopy.events.to_jsonl_line` before being flushed.
...
* :func:`ensure_gitignore_entry` — exposed for tests; called by
  :func:`create_writers`.
* :data:`GITIGNORE_ENTRY` — the literal ``.git-loopy/`` line.
...
* **Idempotent ``.gitignore`` touch.** :func:`ensure_gitignore_entry`
  matches the existing line by ``line.strip() in {".git-loopy/", ".git-loopy"}``
...
def make_run_id(
    *,
    ...
    for ``ls .git-loopy/logs/`` and grep-based log inspection.
...
def ensure_gitignore_entry(repo_root: Path) -> None:
    """Append ``.git-loopy/`` to ``repo_root/.gitignore`` if the entry is missing.
    ...
    * ``.gitignore`` already contains a line whose ``.strip()`` value is
    ...
    for line in existing.splitlines():
        if line.strip() in {".git-loopy/", ".git-loopy"}:
            return
...
class EventLogWriter(AbstractContextManager["EventLogWriter"]):
    ...
    is routed through :func:`git_loopy.events.scrub` and
    :func:`git_loopy.events.to_jsonl_line` (which scrubs again — the
    ...
    def write(self, event: dict[str, Any]) -> None:
        ...
        Args:
            event: Event dict (typically from :func:`git_loopy.events.make_event`).
        """
        if self._fh is None:
...
def create_writers(
    ...
    * Calls :func:`ensure_gitignore_entry` against ``repo_root`` — adds
      ``.git-loopy/`` to ``.gitignore`` if the file exists and the entry is
...
def _format_rfc3339_ms(dt: datetime) -> str:
    ...
    Matches the JSONL envelope's ``ts`` format from :mod:`git_loopy.events`
...
def _build_diagnostics_logger(run_id: str, log_path: Path) -> logging.Logger:
    ...
    first-call's path.
    """
    logger = logging.getLogger(f"git_loopy.diagnostics.{run_id}")
