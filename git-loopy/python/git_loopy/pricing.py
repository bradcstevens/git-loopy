"""``git_loopy.pricing`` — provider-list-price cost estimation and context utilisation.
...
* :func:`load_pricing` — resolve explicit path → ``GIT_LOOPY_PRICING_FILE`` env var →
...
_ENV_OVERRIDE = "GIT_LOOPY_PRICING_FILE"
...
def _packaged_path() -> Path:
    ...
    """
    return Path(str(files("git_loopy") / "pricing.toml"))
...
def load_pricing(path: Path | None = None) -> Pricing:
    ...
    2. The ``GIT_LOOPY_PRICING_FILE`` environment variable, if set and non-empty.
    3. The packaged ``git-loopy/pricing.toml``.
