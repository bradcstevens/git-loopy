...
* **Unknown is not zero, and unknown cannot prove anything.** #219 §11 forbids
  ...
  no contraction *and* fails the health test, which makes the static-safe
  ``min(cap, 3)`` a ceiling and not merely a starting value: a Run that cannot
  see every budget may still contract on what it *can* observe — the
  **Integration backlog** is its own state and always authoritative — but may
  never climb back above the safe default. That is the reading of #219 §6's
  "freeze at ``min(configured Lane cap, 3)``" row that also keeps the pinned
  ``concurrency-changed-unknown-and-observed-none`` conformance case reachable,
  since that case is a contraction reported next to two unknown budgets.
...
class ConcurrencyController:
    ...
    def _recover(self) -> LimitChange | None:
        ...
        if self._healthy_streak < HEALTHY_OBSERVATIONS:
            return None
        if self._effective >= self.configured_lane_cap:
            # #219 §6: the configured **Lane cap** is a strict upper bound and
            # never mutates. Nothing the controller observes can raise it.
            return None
    ...
    def _healthy(self) -> bool:
        ...
        Every clause is a claim the Run must be able to *prove*, which is why
        an unavailable budget fails the test rather than being waived: the
        static-safe limit is a ceiling for a Run that cannot see, not merely
        the value it happens to start on.
        """
        latest = self._window[-1]
        if latest.rate_limits != 0 or latest.parked > 0 or not latest.demand:
            return False
        ...
        if credit is None or host is None:
            return False
        return credit < HEALTHY_RATIO and host < HEALTHY_RATIO
