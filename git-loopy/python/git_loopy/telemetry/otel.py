"""``git_loopy.telemetry.otel`` — the OpenTelemetry seam.

A **single switch** module that the rest of ``git-loopy`` uses for span
...
* ``OTEL_EXPORTER_OTLP_ENDPOINT`` env var is set to a non-empty string —
  ...
  collector enable git-loopy's tracing the same way they enable any other
  Python OTel client.
* ``GIT_LOOPY_OTEL_ENABLED`` env var is set to a truthy value (``"1"``,
...
half-emitted span tree (e.g. a test resetting ``GIT_LOOPY_OTEL_ENABLED``
between iterations of the same loop would otherwise leave the
``git_loopy.run`` root span open while subsequent ``span()`` calls
...
The runner emits this tree::
    ...
    git_loopy.run                  (root, per git-loopy invocation)
...
SDK-emitted spans nest under ``git_loopy.session`` because the Copilot
...
::
    ...
    with telemetry.span("git_loopy.iteration", iter=iter_num) as sp:
        ...
...
:class:`ImportError`, logs a warning via the ``git_loopy.telemetry``
...
_logger = logging.getLogger("git_loopy.telemetry")
...
def _detect_enabled() -> bool:
    ...
    if _is_truthy(os.environ.get("GIT_LOOPY_OTEL_ENABLED")):
        return True
...
def _build_tracer() -> Any:
    ...
    * Otherwise, install a fresh :class:`TracerProvider`. If
      ...
      ``GIT_LOOPY_OTEL_ENABLED=1`` is set, install a provider with no
    ...
    return trace.get_tracer("git_loopy")
...
def _init() -> None:
    ...
    if enabled:
        try:
            ...
            _logger.warning(
                "OTel was requested (GIT_LOOPY_OTEL_ENABLED or "
                "OTEL_EXPORTER_OTLP_ENDPOINT is set) but the "
                "[otel] extra is not installed: %s. "
                "Install with `uv sync --project git-loopy/python --extra otel` "
                "(or `pip install 'git-loopy[otel]'`) to enable tracing.",
                exc,
            )
            _state["tracer"] = None
...
def span(name: str, **attrs: Any) -> Iterator[Any]:
    ...
    Args:
        name: The span name. Convention: dotted, lowercase, prefixed with
            ``git_loopy.`` (matches the contract in the module docstring).
    ...
    Example::
        ...
        with telemetry.span("git_loopy.iteration", iter=1) as sp:
            ...
            sp.set_attribute("issue", first_ref)
    """
    _init()
...
def build_sdk_telemetry_config() -> Optional[dict[str, Any]]:
    ...
    * If only ``GIT_LOOPY_OTEL_ENABLED=1`` is set (no endpoint), the dict
...
def force_flush(timeout_millis: int = 30_000) -> None:
    ...
    outer ``finally`` block (in :func:`git_loopy.loop.run`) so that
...
def reset_for_tests() -> None:
    ...
    Intended for test fixtures that need to toggle ``GIT_LOOPY_OTEL_ENABLED``
