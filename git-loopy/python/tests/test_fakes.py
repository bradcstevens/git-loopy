...
from git_loopy.gate import GateRunner, LoopFailure
from git_loopy.gh import GhError, GitHubClient, Issue, PullRequest, Repo
from git_loopy.git import Commit, GitClient, GitError
...
def test_fake_worktrees_have_independent_logs_from_each_other(tmp_path: Path) -> None:
    ...
    a = parent.add_worktree(tmp_path.parent / "wt-a", branch="git-loopy/R/issue-1", base="main")
    b = parent.add_worktree(tmp_path.parent / "wt-b", branch="git-loopy/R/issue-2", base="main")
...
def test_fake_remove_worktree_is_recorded_and_drops_the_worktree(
    ...
    parent.add_worktree(wt_path, branch="git-loopy/R/issue-7", base="main")
...
def test_fake_worktree_child_satisfies_gitclient_protocol(tmp_path: Path) -> None:
    ...
    parent = FakeGitClient(tmp_path)
    lane = parent.add_worktree(tmp_path.parent / "wt", branch="git-loopy/R/issue-1", base="main")
    assert isinstance(lane, GitClient)
...
def test_fake_merge_raises_for_unknown_branch(tmp_path: Path) -> None:
    ...
    parent = FakeGitClient(tmp_path)
    with pytest.raises(GitError):
        parent.merge("git-loopy/R/issue-999")
...
def test_fake_delete_branch_raises_for_unknown_branch(tmp_path: Path) -> None:
    ...
    parent = FakeGitClient(tmp_path)
    with pytest.raises(GitError):
        parent.delete_branch("git-loopy/R/issue-999")
...
def test_fake_merge_conflict_raises_and_leaves_base_untouched(tmp_path: Path) -> None:
    ...
    int_branch = "git-loopy/R/integrate/issue-7"
