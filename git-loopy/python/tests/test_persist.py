"""Tests for ``git_loopy.persist`` (issue #7).
...
* :func:`make_run_id` — 26-char Crockford-base32 ULID, deterministic
  ...
  prefix sortably.
* :func:`ensure_gitignore_entry` — appends ``.git-loopy/`` when missing,
  no-ops when ``.gitignore`` absent, idempotent when the entry already
  exists in either ``.git-loopy/`` or ``.git-loopy`` form.
...
* Module imports only stdlib + ``git_loopy.events`` (AST guard).
"""
...
def test_ensure_gitignore_entry_appends_when_missing(tmp_path: Path) -> None:
    ...
    assert ".git-loopy/" in content
    # Should be appended after the existing content.
    assert content.rstrip().splitlines()[-1] == ".git-loopy/"
...
def test_ensure_gitignore_entry_adds_leading_newline_when_no_trailing_newline(
    ...
    assert lines == ["node_modules", ".git-loopy/"]
...
def test_ensure_gitignore_entry_idempotent_when_git_loopy_with_slash_present(
    tmp_path: Path,
) -> None:
    gi = tmp_path / ".gitignore"
    gi.write_text("node_modules/\n.git-loopy/\n", encoding="utf-8")
    ...
    content = gi.read_text(encoding="utf-8")
    assert content.count(".git-loopy/") == 1


def test_ensure_gitignore_entry_idempotent_when_git_loopy_without_slash_present(
    tmp_path: Path,
) -> None:
    """Both ``.git-loopy/`` and ``.git-loopy`` are recognised — gitignore treats
    directory-only and "either type" forms as equivalent for our purposes."""
    gi = tmp_path / ".gitignore"
    gi.write_text(".git-loopy\n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    content = gi.read_text(encoding="utf-8")
    # Should NOT append a duplicate '.git-loopy/' line.
    assert content.count(".git-loopy") == 1
    assert ".git-loopy/" not in content
...
def test_ensure_gitignore_entry_tolerates_whitespace_around_existing_line(
    ...
    gi.write_text("  .git-loopy/  \n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    assert gi.read_text(encoding="utf-8").count(".git-loopy/") == 1


def test_ensure_gitignore_entry_does_not_treat_substring_as_match(
    tmp_path: Path,
) -> None:
    """``my.git-loopy/`` (a longer line containing our pattern) should not block
    the append — only the exact stripped match counts."""
    gi = tmp_path / ".gitignore"
    gi.write_text("my.git-loopy/\n", encoding="utf-8")
    ensure_gitignore_entry(tmp_path)
    content = gi.read_text(encoding="utf-8")
    # Both lines now present.
    assert "my.git-loopy/" in content
    assert any(line.strip() == ".git-loopy/" for line in content.splitlines())
...
def test_event_log_writer_flushes_after_each_write_via_subprocess_crash(
    ...
    script = textwrap.dedent(f"""
        ...
        from git_loopy.events import make_event
...
def test_create_writers_aligns_filenames_across_artefacts(tmp_path: Path) -> None:
    ...
    stem so an operator can correlate by ``ls .git-loopy/{logs,runs}/<stem>.*``."""
    bundle = create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
...
def test_create_writers_paths_under_dot_git_loopy(tmp_path: Path) -> None:
    bundle = create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
    assert bundle.event_log.path.parent == tmp_path / ".git-loopy" / "logs"
    assert bundle.run_summary.path.parent == tmp_path / ".git-loopy" / "runs"
    assert bundle.diagnostics_path.parent == tmp_path / ".git-loopy" / "logs"
...
def test_create_writers_gitignore_touch_is_idempotent(tmp_path: Path) -> None:
    """Running the factory twice does not duplicate the .git-loopy/ line."""
    (tmp_path / ".gitignore").write_text("node_modules/\n", encoding="utf-8")
    create_writers(tmp_path, run_id=_FIXED_RUN_ID, started_at=_FIXED_TS)
    create_writers(
        tmp_path,
