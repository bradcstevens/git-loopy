"""Reusable test doubles (fakes) for the ``git-loopy`` seams.
...
class FakeGitClient:
    """Stateful in-memory :class:`~git_loopy.git.GitClient` for loop tests.
...
class FakeGitHubClient:
    """Stateful in-memory :class:`~git_loopy.gh.GitHubClient` for source/loop tests.
    ...
    assertions, and injects per-method :exc:`~git_loopy.gh.GhError` failures so a
    ...
    **Strike** progress or interpret close-keywords. That policy stays in
    :class:`git_loopy.sources.GitHubIssueSource`, never in the client.
    ...
    **List and single-view are independently scriptable.** ``issue_list`` /
    ...
    ``*_view_errors`` / ``issue_close_errors`` inject a :exc:`~git_loopy.gh.GhError`
    ...
    def issue_comment(self, number: int, comment: str) -> None:
        ...
        :exc:`~git_loopy.gh.GhError` so a test drives the source's
...
class FakeGateRunner:
    """Scriptable in-memory :class:`~git_loopy.gate.GateRunner` for Integration tests.
