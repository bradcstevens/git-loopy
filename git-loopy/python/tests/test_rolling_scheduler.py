...
def test_the_pin_is_awaited_from_its_reservation_until_it_binds() -> None:
    scheduler, _source, clock = _scheduler_with_clock([13, 11], lane_cap=2)
    scheduler.pool.lane_first = lambda: 13
    scheduler.start()
    before = scheduler.lane_first_awaited
    (reservation,) = scheduler.reserve()
    in_setup = scheduler.lane_first_awaited

    scheduler.release(reservation, requeue_after=1.0)
    paced = scheduler.lane_first_awaited
    clock.advance(1.0)
    due = scheduler.lane_first_awaited

    assert (before, in_setup, paced, due) == (False, True, True, False)
