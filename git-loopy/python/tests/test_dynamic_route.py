...
def test_dynamic_router_projects_supporting_evidence_without_changing_aa_election() -> (
    ...
    async def fetch_evidence() -> dynamic_route.FreshEvidence:
        return replace(
            ...
            supporting_sources=tuple(
                replace(source, retrieved_at=source.retrieved_at.replace(
                    minute=len(recorded) + len(assessments)
                ))
    ...
    reused = asyncio.run(router.rebind((_reusable_from(decision),), request))
    assert isinstance(reused, dynamic_route.DynamicRouteDecision)
    assert reused.reused is True
    assert reused.work_evidence.supporting_evidence == (supporting,)
    assert len(assessments) == 1

    evidence = replace(
        evidence, supporting_records=(replace(supporting, score=Decimal("73.0")),)
    )
    reassessed = asyncio.run(router.rebind((_reusable_from(reused),), request))
    assert isinstance(reassessed, dynamic_route.DynamicRouteDecision)
    assert reassessed.reassessed is True
    assert "SWE-bench Verified 73.0%" in reassessed.summary
