...
# command that actually *executes* that member's suite. Naming the member's
# directory is not enough — `bash -n` names the shell tests without running one,
# and `cargo fmt` names the Rust manifest without compiling a test — so each set
# also pins the invocation that runs it.
FAMILY_MARKERS = {
    "Python Orchestrator": ("pytest", "git-loopy/python/tests"),
    "shell Orchestrator": ("bash", "git-loopy/shell/tests/test-*.sh"),
    "PowerShell Orchestrator": ("pwsh", "git-loopy/powershell/tests/test-*.ps1"),
...
def test_every_runner_family_member_has_a_declared_loop(
    ...
    """An Iteration must be able to run what CI runs, for the whole family.
    ...
    for marker in markers:
        assert marker in commands, (
            f"no feedback loop runs the {member}'s suite; "
