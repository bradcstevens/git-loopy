"""Tests for ``git_loopy.pricing`` (issue #4).
...
* Packaged ``pricing.toml`` carries the date comment, the premium-request
  warning, and the ``GIT_LOOPY_PRICING_FILE`` override note required by the PRD.
...
from git_loopy import pricing as pricing_module
from git_loopy.pricing import (
    ModelPricing,
    Pricing,
    PricingError,
    context_utilisation,
...
def test_load_pricing_env_var_override(
    ...
    monkeypatch.setenv("GIT_LOOPY_PRICING_FILE", str(custom))
    p = load_pricing()
...
def test_load_pricing_explicit_path_beats_env_var(
    ...
    monkeypatch.setenv("GIT_LOOPY_PRICING_FILE", str(env_file))
...
def test_load_pricing_env_var_empty_string_falls_through_to_packaged(
    ...
    monkeypatch.setenv("GIT_LOOPY_PRICING_FILE", "")
    p = load_pricing()
    assert "claude-opus-4.8" in p.models
...
def test_packaged_pricing_toml_documents_env_var_override() -> None:
    text = _packaged_pricing_toml_text()
    assert "GIT_LOOPY_PRICING_FILE" in text
