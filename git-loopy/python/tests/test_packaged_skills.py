...
``git-loopy init`` offers to scaffold git-loopy's agent skills into the chosen
...
canonical copy; ``git-loopy/skills/setup-agent-skills/`` is the vendored copy that
...
# ---------------------------------------------------------------------------
# Wheel packaging: the skill ships as git_loopy/skills/... in the built artifact
# ---------------------------------------------------------------------------
...
def test_skill_is_packaged_into_the_built_wheel(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheels[0]) as zf:
        names = zf.namelist()
    wanted = f"git_loopy/skills/{_SKILL_DIRNAME}/SKILL.md"
