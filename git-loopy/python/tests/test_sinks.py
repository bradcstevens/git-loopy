"""Tests for ``git_loopy.sinks`` (issue #22 — event sink fan-out).
...
:class:`~git_loopy.sinks.SinkFanout` instead of calling the line-printer
:class:`~git_loopy.ui.renderer.Renderer` directly. JSONL logging stays
...
def test_sinks_module_imports_are_constrained() -> None:
    ...
    No ``rich`` / ``textual`` / ``copilot`` SDK / shell-side git-loopy
