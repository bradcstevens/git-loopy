"""Tests for ``git_loopy.git`` (issue #6).
...
pytestmark = pytest.mark.skipif(
    ...
    reason="git not on PATH; git_loopy.git wrappers cannot be exercised",
)
...
def test_commit_preserves_multi_paragraph_message(tmp_path: Path) -> None:
    ...
    assert "GitLoopy-Checkpoint: 32" in recorded.body
...
def test_lane_branch_name_follows_git_loopy_convention() -> None:
    """A Lane's branch is ``git-loopy/<run_id>/issue-<N>`` (ADR-0005 prefix)."""
    assert lane_branch_name("01JAX7QZ8K9M", 7) == "git-loopy/01JAX7QZ8K9M/issue-7"


def test_lane_branch_name_is_pure_string_policy() -> None:
    """Distinct run_ids / issue numbers produce distinct, well-formed branches."""
    assert lane_branch_name("RUNA", 1) == "git-loopy/RUNA/issue-1"
    assert lane_branch_name("RUNB", 123) == "git-loopy/RUNB/issue-123"
