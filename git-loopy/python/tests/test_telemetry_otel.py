"""Unit tests for :mod:`git_loopy.telemetry.otel`.

Covers the seam in isolation — the integration with
:mod:`git_loopy.loop` is exercised by
...
* ``GIT_LOOPY_OTEL_ENABLED=1`` alone → enabled, empty SDK config
...
def fresh_otel(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.delenv("GIT_LOOPY_OTEL_ENABLED", raising=False)
...
def test_is_enabled_returns_true_when_git_loopy_otel_enabled_is_truthy(
    fresh_otel: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """GIT_LOOPY_OTEL_ENABLED=1 → enabled, no endpoint required."""
...
def test_is_enabled_accepts_documented_truthy_values(
    ...
    monkeypatch.setenv("GIT_LOOPY_OTEL_ENABLED", truthy)
    otel.reset_for_tests()
    assert otel.is_enabled() is True
...
def test_is_enabled_rejects_falsy_values(
    ...
    """Falsy / empty GIT_LOOPY_OTEL_ENABLED is treated as disabled."""
    monkeypatch.setenv("GIT_LOOPY_OTEL_ENABLED", falsy)
    otel.reset_for_tests()
    assert otel.is_enabled() is False
...
def test_span_yields_noop_when_disabled(fresh_otel: None) -> None:
    ...
    with otel.span("git_loopy.test", iter=1, issue=42) as sp:
    ...
    with otel.span("git_loopy.other") as sp2:
...
def test_span_does_not_import_opentelemetry_when_disabled(
    ...
    with otel.span("git_loopy.test", iter=1):
        pass
...
def test_span_emits_real_span_when_enabled(
    ...
    with otel.span("git_loopy.test", iter=3) as sp:
        sp.set_attribute("issue", 42)

    spans = in_memory_exporter.get_finished_spans()
    assert len(spans) == 1
    emitted = spans[0]
    assert emitted.name == "git_loopy.test"
...
def test_span_skips_none_attributes(
    ...
    with otel.span("git_loopy.test", iter=1, missing=None):
        pass
...
def test_span_nesting_produces_parent_child_relationship(
    ...
    with otel.span("git_loopy.outer"):
        with otel.span("git_loopy.inner"):
            pass

    spans = in_memory_exporter.get_finished_spans()
    # SimpleSpanProcessor finishes in LIFO; sort by name for stable lookup.
    by_name = {s.name: s for s in spans}
    assert "git_loopy.outer" in by_name
    assert "git_loopy.inner" in by_name
    outer = by_name["git_loopy.outer"]
    inner = by_name["git_loopy.inner"]
...
def test_span_async_friendly_propagation(
    ...
    sync ``with telemetry.span("git_loopy.session"):``. This works
    ...
    async def inner_work() -> None:
        with otel.span("git_loopy.async_inner"):
            await asyncio.sleep(0)

    async def main() -> None:
        with otel.span("git_loopy.async_outer"):
            await inner_work()
    ...
    spans = in_memory_exporter.get_finished_spans()
    by_name = {s.name: s for s in spans}
    inner = by_name["git_loopy.async_inner"]
    outer = by_name["git_loopy.async_outer"]
...
def test_build_sdk_telemetry_config_returns_empty_dict_when_enabled_without_endpoint(
    ...
    """GIT_LOOPY_OTEL_ENABLED=1 alone → SDK gets ``{}`` (enables telemetry env vars).
...
def test_force_flush_calls_provider_force_flush_when_enabled(
    ...
    with otel.span("git_loopy.flush_test"):
        pass
    ...
    spans = in_memory_exporter.get_finished_spans()
    assert any(s.name == "git_loopy.flush_test" for s in spans)
...
def test_init_degrades_gracefully_when_opentelemetry_unavailable(
    ...
    """Operator sets GIT_LOOPY_OTEL_ENABLED=1 but [otel] extra is not installed.
    ...
    with caplog.at_level("WARNING", logger="git_loopy.telemetry"):
        assert otel.is_enabled() is False
        # span() still works (yields no-op).
        with otel.span("git_loopy.test", iter=1) as sp:
            sp.set_attribute("k", "v")
...
def test_module_does_not_import_opentelemetry_at_module_load() -> None:
    ...
    assert "opentelemetry" not in sys.modules, (
        "importing git_loopy.telemetry.otel must not import opentelemetry"
    )
