"""Static guard: no Python-native API libraries in ``git_loopy/``.
...
Our own ``git_loopy.git`` module is **not** affected by this guard because
its top-level package name is ``git_loopy``, not ``git`` — qualified
imports (``from git_loopy import git``, ``from git_loopy.git import head_sha``,
``import git_loopy.git``) and relative imports (``from . import git``) all
target ``git_loopy``.

The check is performed via AST inspection across every ``*.py`` under
``git_loopy/`` so any future module addition is automatically covered.
"""
...
def _iter_package_python_files() -> list[Path]:
    """Return every ``.py`` file under the installed ``git_loopy`` package."""
    pkg_dir = Path(git_loopy.__file__).parent
...
def test_at_least_one_module_is_inspected() -> None:
    ...
    assert files, "expected to find Python modules under git-loopy/ to inspect"


def test_no_forbidden_api_libraries_in_any_git_loopy_module() -> None:
    ...
    * ``from git_loopy import git`` — qualified, top-level package is ``git-loopy``.
    * ``import git_loopy.git`` — same.
    * ``from . import git`` / ``from .git import head_sha`` — relative imports
    ...
    assert not failures, (
        "git-loopy/ must funnel every external API call through subprocess "
        "wrappers around `gh` and `git` — Python-native API libraries are "
        "forbidden:\n  " + "\n  ".join(failures)
    )


def test_our_own_git_loopy_git_module_is_not_misidentified() -> None:
    """Regression guard for the allowlist logic.

    Imports of our own ``git_loopy.git`` module — qualified or relative —
    ...
    """
    samples = [
        "from git_loopy import git",
        "from git_loopy.git import head_sha",
        "import git_loopy.git",
        "from . import git",
    ...
    for src in samples:
        ...
        if isinstance(node, ast.Import):
            ...
            assert top == "git_loopy", f"{src!r} classified wrongly: top={top!r}"
        elif isinstance(node, ast.ImportFrom):
            if node.level > 0:
