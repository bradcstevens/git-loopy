...
| `unbound-run-notice.json` | The **Unbound-Run notice** (#642). Only a Run that ended `empty_pool`, `all_blocked` or `all_skipped` without binding, activating or contributing to an issue earns one; `unbound_run_outcomes` is that set, and each adapter asserts its own constant against it. The fixture pins the exact lines: the exclusions that emptied a Pool; the blockers outside the Pool, which the Run's `repository` input separates from members; and each refusal kind, counted once per member. The Rust Dashboard draws them over a held Dashboard, and the Python attach client prints them after the Dashboard returns |
