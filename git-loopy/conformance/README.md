...
[Wrapper contract](../../docs/wrapper-contract.md), plus the renderer-neutral
presentation that more than one member draws (`dashboard-insights.json`,
`unbound-run-notice.json`). Every Orchestrator must read
...
| `pin-duration.json` | How long a **Pin** lasts (§3.2, contract 2.14, #644): each case is a sequence of **Pickups**, and each step takes the Pool and how that Pickup read the Pin (`bound`, `refused` for an answer about the Pin, `unread`, or not listed by a `complete` or incomplete read) and expects the order it computes, the `reason` a binding of the Pin reports, and whether the Pin is live afterwards. It covers a Pin bound once and still open, one passed over as Blocked and never promoted again, an unread Pin promoted again and bound as `pin`, one absent from a complete read (spent) and from an incomplete read (live), and a spent Pin labelled `priority` that reports `priority`. Eligibility stays out, as in `issue-ordering.json`: the read outcome is the input |
...
| `unbound-run-notice.json` | The **Unbound-Run notice** (#642, #643). Only a Run that ended `empty_pool`, `all_blocked` or `all_skipped` without binding, activating or contributing to an issue earns one; `unbound_run_outcomes` is that set, and each adapter asserts its own constant against it. The fixture pins the exact lines: the exclusions that emptied a Pool; the blockers outside the Pool, which the Run's `repository` input separates from members; and each refusal kind, counted once per member. Contract 2.12 adds Run-end `refusals` as the Pool and skip record when present, ahead of any collection; a Membership read remains insufficient. The Rust Dashboard draws the notice over a held Dashboard, and the Python attach client prints it after the Dashboard returns |
...
| `release-line.json` | `vX.Y.Z` **Release-target** labels resolved against the last stable Release into the closed four-key **Bump class** taxonomy — no label is `none`, and malformed, conflicting and unreachable labels are refused; the **Release target** ratchet, the `alpha`/`beta`/`rc` prerelease stages and counter, operator stage advances and their refusals, milestone Promotion from any stage, the Pickup label, and Integration-order-independent Release-line outcomes (ADR-0066) |
...
`fixture-claims.json` is the family-accounting register, not a fixture: it
...
renderer's production seam exposes on both sides: the Rust `ActivityBand` takes one as a
...
Native ports do not implement routing yet, so these four fixtures are
Python-adapter-only.
...
A case may declare a `concurrency`, which is how many **Trials** the search buys
at a time (#381). The adapter drives the *inline* dispatcher at that width rather
than the threaded one, so a case pins **what is bought and what it is charged**
without pinning a thread schedule: the search cannot tell the two dispatchers
apart, and a fixture that raced would pin nothing at all. `expected.dispatches`
is the sizes it bought in, which is the whole of the probe rule — one Trial
alone, then the remainder at the operator's width — and the pair of
`the-wall-clock-ceiling-bounds-elapsed-time-and-not-summed-trial-time` and
`the-same-walk-run-serially-reaches-that-wall-clock-ceiling` is the same script
at two widths, where only the elapsed time and therefore the outcome differ.
