...
| `issue-ordering.json` | The **total order** over eligible issues (§3.2): `priority` rank ahead of age, `created_at` ascending, issue number as the tie-break that makes the order total, the narrow timestamp grammar and the `absent`/`malformed` defect an undated issue is reported with, and the head of the order each case selects. Eligibility is deliberately absent — that is `discriminator.json`'s decision, and a second home for it is a second place it can drift |
