...
| `exit-codes.json` | Clean, aborted, and usage-error process exits — including the `all_skipped` abort (§10, #413) a Pickup owes when the Pool is non-empty and it could bind none of it — plus `pool_emptiness_cases`, the one rule every member asks before claiming the exit-`0` empty Pool (§2.2, #541): only a *complete* read that found nothing establishes emptiness, so a failed or truncated read that found nothing is unknown rather than empty. The rule rides this fixture rather than one of its own because it decides which of these exits a Run is entitled to |
