...
| `no-work-notice.json` | The operator notice a Run that found nothing it could work ends with (#642): only a Run that ended `empty_pool`, `all_blocked` or `all_skipped` without binding, activating or contributing to an issue earns one. The fixture pins its exact lines: the root blockers outside the Pool, and each refusal kind counted once per candidate. The Rust Dashboard draws the lines over a held Dashboard, and the Python attach client prints them after the Dashboard returns |
