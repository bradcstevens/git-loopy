...
version. Both URLs are pinned against the helper manifest's repository, so a
fork or a rename cannot leave every channel downloading from the old
repository's Releases. The shell adapter is
