...
| **Perl 5** | The native Continuation command uses core `JSON::PP`, `Encode`, and `Unicode::Normalize` to enforce the portable JSON profile without Python. It is also how a Run restores a **default SIGPIPE** for the agent process when the Orchestrator was launched by a parent that ignores it (CI runners do): Bash can never lift an ignore it was handed. Without `perl` the Run still starts — it says so once and the agent inherits the ignore. |
