...
declare -Ar GIT_LOOPY_EVENT_TYPES=(
  ...
  # deferred rather than silently claimed. Carrying the literals is not a claim
  # that it routes -- `GIT_LOOPY_INSIGHT_CAPABILITIES_JSON` below still says
  ...
  # Orchestrator schedules no Lane, so it never degrades from one. Declared so
  # the table stays the whole event vocabulary and its replay reads.
  ...
  # binary and subscribes to no SDK event stream, so it observes no failure
  # record to map. Recording only: neither literal implies that any Orchestrator
  # aborts, waits, or retries because it saw one.
