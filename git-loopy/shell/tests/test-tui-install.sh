...
bounded_index="$cli_dir/bounded-index"
mkdir -p "$bounded_index"
jq -n '[range(100) | {"draft": true, "assets": []}]' >"$bounded_index/full.json"
for ((page = 1; page < 10; page++)); do
  cp "$bounded_index/full.json" "$bounded_index/releases-$page.json"
done
printf '[]\n' >"$bounded_index/releases-10.json"
jq --arg index "file://$bounded_index/releases-{page}.json" \
  '.release_index_url_template = $index' "$fallback_metadata" \
  >"$bounded_index/metadata.json"
assert_equal "[]" "$(
  git_loopy_tui_published_releases "$bounded_index/metadata.json" "$host_archive" "$host_checksum"
)" "a Release index that ends on the tenth page completes"
cp "$bounded_index/full.json" "$bounded_index/releases-10.json"
if bounded_output="$(
  git_loopy_tui_published_releases "$bounded_index/metadata.json" "$host_archive" "$host_checksum" 2>&1
)"; then
  fail "an incomplete Release index was accepted"
fi
assert_contains "$bounded_output" "pagination limit" \
